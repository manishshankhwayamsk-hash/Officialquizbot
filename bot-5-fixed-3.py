...
# AI generation (needed for /topic, /pdf, /image, /section)
# These are the bot OWNER's fallback keys — used for anyone who hasn't added their
# own with /addapi. Set ANY of these env vars you have; every one you set becomes an
# extra fallback that EVERY user gets automatically, tried in OWNER_FALLBACK_ORDER
# below, on top of each user's own personal /addapi keys (which are always tried
# FIRST — owner keys are the safety net, not the first choice).
#
# Got MULTIPLE keys for the same provider (e.g. several Groq accounts)? Put them all
# in the same env var separated by commas — e.g. GROQ_API_KEY=key1,key2,key3 — and
# every one becomes its own fallback entry, tried in the order listed, each with its
# own independently-tracked daily usage/alerts (see /limits).
#
# Anthropic has NO ongoing free API tier (only a one-time ~$5 signup credit) — so if
# you want a genuinely free, always-on shared key, set GROQ_API_KEY / GEMINI_API_KEY /
# CEREBRAS_API_KEY / NVIDIA_API_KEY instead (or alongside). ANTHROPIC_API_KEY still
# works exactly as before if you do have a paid key — it's just no longer the ONLY
# fallback option.
...
def _split_keys(raw: str) -> list:
    """'key1, key2,key3' -> ['key1','key2','key3']; '' -> []. Lets any of the env
    vars below hold either one key or several comma-separated ones interchangeably."""
...
OWNER_PROVIDER_KEYS = {
    ...
    "nvidia": {"api_keys": _split_keys(os.getenv("NVIDIA_API_KEY", ""))},
    "cloudflare_text": {
    ...
    "deepseek": {"api_keys": _split_keys(os.getenv("DEEPSEEK_API_KEY", ""))},
    "openai": {"api_keys": _split_keys(os.getenv("OPENAI_API_KEY", ""))},
    "anthropic": {"api_keys": _split_keys(ANTHROPIC_API_KEY)},
}
# Cheapest/most-generous-and-genuinely-free providers first; paid ones (openai,
# anthropic) last since owner keys are meant to be the free safety net. DeepSeek
# sits near the end of the free ones since its free tokens are a one-time signup
# credit (not a daily-reset allowance like cerebras/groq/gemini/nvidia above it).
OWNER_FALLBACK_ORDER = [
    "cerebras", "groq", "gemini", "nvidia", "cloudflare_text", "cloudflare_vision",
    "github", "openrouter_text", "openrouter_vision", "mistral", "huggingface",
...
PROVIDER_PRESETS = {
    ...
    "gemini": {
        ...
        "model": "gemini-2.5-flash",  # gemini-2.0-flash was retired by Google on 2026-06-01
    },
    "openai": {
        "label": "🟣 OpenAI",
        "style": "openai",
    ...
    "nvidia": {
        ...
        "note": "Free dev account, card nahi chahiye — build.nvidia.com se key lo (~40 req/min tak). Text-only.",
    },
    "deepseek": {
        "label": "🐋 DeepSeek",
        "style": "openai",
        "api_base": "https://api.deepseek.com/v1",
        "model": "deepseek-chat",  # DeepSeek keeps this alias pointed at their current
        # general model as they upgrade it — if it ever 404s, check platform.deepseek.com
        # for the current model name and update this one line.
        "note": "Naye account ko free signup tokens milte hain — card nahi chahiye. Text-only.",
    },
    "huggingface": {
    ...
    "custom": {
        ...
        "note": "Koi bhi OpenAI-compatible provider yahi se add karo — jaise AI21 "
                "(base https://api.ai21.com/studio/v1, model jamba-mini — $10 free "
                "trial credit, card nahi chahiye) ya Pollinations (base "
                "https://gen.pollinations.ai/v1, model openai — free key enter.pollinations.ai "
                "se). Base URL + apni key + model naam us provider ke docs se le lo.",
    },
}
...
(
    ...
    ASK_AI_TIMER, ASK_AI_SHUFFLE,
    IMPORT_QUIZ_POLL, IMPORT_QUIZ_TAG, IMPORT_QUIZ_DESC, IMPORT_QUIZ_PROMO,
) = range(35)
...
# ==========================================
# 🧬 SIGNATURE UI SYSTEM — unique across every screen in the bot
# ==========================================
# One shared visual language, built from three ideas mixed together:
#   • terminal/hacker   → block-bars, 『 bracket 』 titles, ▸ line markers
#   • neon-cyber        → bold icon accents, status dots (🟢🟡🔴)
#   • clean boxed       → consistent header/footer frame on every message
# Every screen below is built from these few primitives so the whole bot
# reads as one coherent, unmistakable UI instead of ad-hoc emoji + bold text.

UI_BAR = "▰" * 18


def ui_header(title: str, icon: str = "⚡") -> str:
    """Signature top banner for every screen: block-bar + bracketed title."""
    return f"{UI_BAR}\n『 {icon}  {stylize(title)}  {icon} 』\n{UI_BAR}"


def ui_footer(tag: str = None) -> str:
    """Signature bottom banner — closes out a screen with the branded tagline."""
    return f"{UI_BAR}\n»» _{tag or FONT_STYLE}_ ««"


def ui_row(label: str, value) -> str:
    """One terminal-style data line: ▸ Label → value"""
    return f"▸ **{label}** → `{value}`"


def ui_bullet(text: str, icon: str = "▸") -> str:
    """One terminal-style bullet line, for lists/instructions."""
    return f"{icon} {text}"


def ui_status(text: str, ok: bool = True) -> str:
    """A status line with a neon dot + terminal-style [ OK ]/[ ERR ] tag."""
    dot, tag = ("🟢", "[ OK ]") if ok else ("🔴", "[ ERR ]")
    return f"{dot} {tag} {text}"


# ------------------------------------------------------------------
# ⌨️ LIVE-TYPING EFFECT
# ------------------------------------------------------------------
# Reveals text word-by-chunk (with Telegram's real "typing…" indicator
# between chunks) instead of dropping the whole message instantly, so it
# reads like someone is live-typing the reply.
#
# Native Telegram Polls are the one thing this CANNOT do — the Bot API has
# no way to edit a poll's question/options after it's sent (only stopPoll,
# which just closes it), so a poll always appears fully-formed in one shot.
# send_typed_poll_intro() gets as close as the API allows: it live-types the
# question as an ordinary message first, holds a beat, deletes it, and the
# real poll drops in right after — same anticipation, honest about the limit.
async def send_typed(bot, chat_id, text: str, chunk_words: int = 5, delay: float = 0.25, **kwargs):
    words = text.split(" ")
    ...
    try:
        await bot.send_chat_action(chat_id, "typing")  # sent once — Telegram shows it for ~5s on its own
        shown = " ".join(words[:chunk_words])
        msg = await bot.send_message(chat_id, shown + " ▌")
        for i in range(chunk_words, len(words), chunk_words):
            await asyncio.sleep(delay)
            shown += " " + " ".join(words[i:i + chunk_words])
            cursor = " ▌" if i + chunk_words < len(words) else ""
            try:
                await bot.edit_message_text(chat_id=chat_id, message_id=msg.message_id, text=shown + cursor)
            except Exception:
                pass
        # Final pass applies real formatting/reply_markup that plain-text edits above skipped.
        if kwargs:
            try:
                await bot.edit_message_text(chat_id=chat_id, message_id=msg.message_id, text=text, **kwargs)
            except Exception:
                pass
        return msg
    except Exception:
        # The animation is cosmetic only — never let it swallow the actual message.
        return await bot.send_message(chat_id, text, **kwargs)


async def send_typed_poll_intro(bot, chat_id, question_preview: str, hold: float = 0.4):
    """Lightweight on purpose: this fires once per question in a live quiz, so
    it stays to ONE chat_action + ONE message + ONE delete (3 calls total,
    ~0.6s) instead of the word-by-word chunking send_typed() does — that
    version was adding several extra API round-trips and real delay to every
    single question, which is what made a multi-question live quiz feel slow
    on a low-CPU free host."""
    try:
        await bot.send_chat_action(chat_id, "typing")
        await asyncio.sleep(hold)
        placeholder = await bot.send_message(chat_id, f"❓ {question_preview}"[:300])
        await asyncio.sleep(0.2)
        await bot.delete_message(chat_id, placeholder.message_id)
    except Exception as e:
        logger.warning(f"Typed poll intro skipped (non-fatal): {e}")
...
class _PGConnShim:
    """Wraps a pooled psycopg2 connection. close() does NOT actually close the
    underlying connection — it hands it back to the shared pool so the next
    db_connect() call can reuse it instead of paying for a fresh TCP+SSL
    handshake to Supabase every single time. This is a drop-in replacement:
    every existing `conn = db_connect(); ...; conn.close()` call site across
    the codebase keeps working exactly as before, unmodified."""
    ...
    def close(self):
        ...
        self._returned = True
        try:
            # A connection left mid-transaction must not go back to the pool
            # dirty — roll back any uncommitted work before returning it.
            if self._conn.status != psycopg2.extensions.STATUS_READY:
                self._conn.rollback()
            self._pool.putconn(self._conn)
        except Exception:
            # Connection is unhealthy (e.g. pooler dropped it) — discard it
            # entirely rather than returning a broken connection to the pool.
            try:
    ...
    def __del__(self):
        # SAFETY NET: none of the ~94 call sites across this file wrap
        # `conn = db_connect(); ...; conn.close()` in try/finally, so any
        # exception raised between connect and close previously leaked that
        # connection out of the pool forever. With maxconn=10, a handful of
        # leaked exceptions was enough to exhaust the pool — after that every
        # future db_connect() started raising `PoolError: connection pool
        # exhausted` (or, on some psycopg2 versions, blocking), which reads to
        # a user as "bot ruk gaya" even though the process itself was still
        # alive. This finalizer returns the connection the moment Python
        # garbage-collects an un-closed shim, so a leak self-heals instead of
        # accumulating. It's a backstop, not a substitute for calling
        # .close() — CPython usually collects immediately once the shim goes
        # out of scope, but don't rely on exact timing.
...
def _get_pg_pool():
    """Lazily creates the shared connection pool on first use (module import
    time is too early — env vars / DATABASE_URL must already be loaded)."""
    global _PG_POOL
    if _PG_POOL is None:
        # maxconn raised 10 -> 20: the __del__ safety net above should stop
        # leaks from accumulating, but a bit of extra headroom means a brief
        # burst of leaked/slow connections doesn't immediately start throwing
        # PoolError at users while GC catches up.
...
def _get_pooled_pg_connection():
    """Grabs a connection from the pool and makes sure it's actually alive
    before handing it back — Supabase's pooler can silently drop idle
    backend connections, and a dead connection needs to be discarded and
    replaced rather than reused (or every query on it would fail).

    Also retries briefly on PoolError ("connection pool exhausted") instead
    of failing the request instantly: a leaked connection is now reclaimed
    by _PGConnShim.__del__ as soon as Python garbage-collects it, which is
    usually near-instant but not guaranteed to be synchronous with the next
    caller's getconn(). A short retry window absorbs that race instead of
    surfacing a hard error to the user for what is really a momentary gap."""
    pool = _get_pg_pool()
    conn = None
    last_err = None
    for attempt in range(5):
        try:
            ...
            last_err = e
            gc.collect()  # nudge GC to reclaim any leaked _PGConnShim now
            time.sleep(0.3)
    if conn is None:
        logger.error(f"DB pool exhausted after retries: {last_err}")
        raise last_err
    try:
...
def db_connect():
    ...
    Postgres connections come from a small shared pool (see _PGConnShim above)
    instead of opening a brand-new connection on every call — that per-call
    handshake to a remote Supabase host was the single biggest source of
    ...
    if USE_POSTGRES:
        return _PGConnShim(_get_pooled_pg_connection(), _get_pg_pool())
...
def init_db():
    ...
    cursor.execute("""
        CREATE TABLE IF NOT EXISTS users (
            ...
            question_limit INTEGER DEFAULT 999999,
    ...
    # 🤺 Duel Mode — one row per challenge, so a scheduled job can look up who was
    # dueling once their live quiz session finishes and post a head-to-head result.
    cursor.execute(f"""
    ...
    # ☀️ Groups subscribed to the automatic daily "Quiz of the Day"
    cursor.execute("""
        CREATE TABLE IF NOT EXISTS daily_quiz_subs (
    ...
    # 💬 /feedback — simple inbox so users can message the owner without needing
    # the owner's personal contact info
    cursor.execute(f"""
    ...
    # "Advance" sticker game (part of Krish Mode) — a shared pool of stickers
    # users send in, so the bot can reply with someone else's sticker later and
    # feel like a real person is chatting back. set_name is kept so an obviously
    # bad pack can be purged later; the blocklist check happens BEFORE a row is
    # ever inserted (see ADVANCE_STICKER_BLOCKLIST below).
    cursor.execute(f"""
        CREATE TABLE IF NOT EXISTS advance_stickers (
            id {PK_TYPE},
            file_id TEXT UNIQUE,
    ...
    # 🎯 Weak-Topic Auto-Revision — every wrong answer a user gives (in any live
    # quiz or duel) is logged here so "Revise My Mistakes" can rebuild a fresh
    # quiz out of just the things they got wrong. Capped per user in code (not
    # here) so this can't grow forever for a very active user.
    cursor.execute(f"""
    ...
    # 🏆 Squad Tournament — one row per group-vs-group challenge on a shared quiz.
    # Each side's combined score is filled in once that group finishes playing;
    # the result is announced once both sides are in.
    cursor.execute(f"""
        CREATE TABLE IF NOT EXISTS tournaments (
            id {PK_TYPE},
            quiz_id INTEGER,
            group_a_id BIGINT,
            group_a_name TEXT DEFAULT '',
            group_a_score INTEGER DEFAULT NULL,
            group_b_id BIGINT DEFAULT NULL,
            group_b_name TEXT DEFAULT '',
            group_b_score INTEGER DEFAULT NULL,
            created_by BIGINT,
    ...
    # ⏰ Time Capsule Quiz — a saved quiz that fires exactly ONCE at a future
    # date/time to a specific chat (unlike scheduled_quizzes, which repeats on a
    # fixed interval to every admin group). A restart-safe one-shot: the row just
    # sits here until a periodic job finds it's due.
    cursor.execute(f"""
    ...
    # 🤝 Study Buddy Match — an opt-in queue; whoever's next in line with a
    # similar accuracy gets matched and both are notified. Rows are deleted the
    # moment a match is made, so this table only ever holds people still waiting.
    cursor.execute(f"""
        CREATE TABLE IF NOT EXISTS study_buddy_queue (
            user_id BIGINT PRIMARY KEY,
    ...
    # 📢 Owner broadcasts scheduled for a future time (run once, then removed)
    cursor.execute(f"""
    ...
    # 🤖 AI MODE — maps every native quiz poll AI Mode sends to its correct answer +
    # origin group, so the single shared PollAnswerHandler can score it even after a
    # bot restart (poll answers can arrive minutes/hours after the poll was sent).
    # Rows older than a week are pruned automatically (see _prune_old_ai_mode_polls).
    cursor.execute("""
        CREATE TABLE IF NOT EXISTS ai_mode_polls (
    ...
    # 🤖 AI MODE — running per-group, per-UTC-day score for every user who answers an
    # AI Mode poll (+5 correct / -6 wrong, see AI_MODE_CORRECT_POINTS/WRONG_POINTS).
    # Kept keyed by score_date (not reset/deleted) so the daily leaderboard job always
    # has a clean "today" bucket per group and past days remain available in the DB.
    cursor.execute("""
        CREATE TABLE IF NOT EXISTS ai_mode_scores (
            chat_id BIGINT,
            user_id BIGINT,
            user_name TEXT,
            score_date TEXT,
            ...
            wrong_count INTEGER DEFAULT 0,
            PRIMARY KEY (chat_id, user_id, score_date)
        )
    """)
...
BACKUP_TABLES = [
    ...
    "duels", "daily_quiz_subs", "feedback", "scheduled_broadcasts",
    "advance_stickers", "wrong_answers", "tournaments", "time_capsules", "study_buddy_queue",
]
...
def restore_from_backup(payload: dict) -> dict:
    ...
    Table-by-table and row-by-row: one bad/old row (extra column from a schema
    that's since changed, a stale FK, etc.) is skipped instead of aborting the
    whole restore and leaving the DB connection mid-transaction — that half-open
    state is what used to lock the DB and take every other command down with it.
    """
    key_cols = {
        ...
        "user_channels": ["id"], "scheduled_quizzes": ["id"], "duels": ["id"],
        "daily_quiz_subs": ["chat_id"], "feedback": ["id"], "scheduled_broadcasts": ["id"],
        "advance_stickers": ["id"], "wrong_answers": ["id"], "tournaments": ["id"],
        "time_capsules": ["id"], "study_buddy_queue": ["user_id"],
    }
    conn = db_connect()
    summary = {}
    try:
        for table, rows in payload.get("tables", {}).items():
            ...
            if not keys:
                summary[table] = f"skipped — unknown table, no key mapping"
                continue

            # Only write columns that actually exist in *this* DB's copy of the
            # table — a backup taken from an older/newer schema (a column added
            # or renamed since) must not blow up the whole restore.
            cur = conn.cursor()
            cur.execute(f"SELECT * FROM {table} LIMIT 0")
            valid_cols = {d[0] for d in cur.description}

            restored = 0
            skipped = 0
            for row in rows:
                row = {c: val for c, val in row.items() if c in valid_cols}
                ...
                try:
                    row_cur = conn.cursor()
                    upsert(row_cur, table, k, v)
                    conn.commit()
                    restored += 1
                except Exception as e:
                    conn.rollback()
                    skipped += 1
                    logger.warning(f"Restore: skipped a {table} row ({k}): {e}")
            summary[table] = restored if not skipped else f"{restored} row(s), {skipped} skipped"
    finally:
        conn.close()

    # Restoring rows with their original explicit `id` values (to keep FKs like
    # quiz_id/set_id valid) leaves each SERIAL column's underlying sequence
    # pointing at wherever it was *before* the restore — the next normal INSERT
    # then reuses an id that already exists → "duplicate key value violates
    # unique constraint ..._pkey". Bump every sequence right after restoring.
...
# ==========================================
# ☀️ /dailyquiz — auto-posts a fresh quiz to a subscribed group every day at a
# chosen UTC hour. Checked hourly; a `last_sent_date` guard means it only actually
# fires once per UTC day per group even though the check itself runs every hour.
# ==========================================
async def dailyquiz_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if update.effective_chat.type not in ("group", "supergroup"):
        await update.message.reply_text("☀️ /dailyquiz sirf group me chalta hai.")
        return
    if not await is_chat_admin(update, context):
        await update.message.reply_text("⛔ Sirf group admins ye set kar sakte hain.")
        return
    if not context.args:
        await update.message.reply_text(
            "Usage:\n"
            "`/dailyquiz on <topic>` — daily quiz on (default 9 AM UTC)\n"
            "`/dailyquiz off` — daily quiz band karo\n"
            "`/dailyquiz hour <0-23>` — UTC hour badlo",
            parse_mode="Markdown",
        )
        return
    ...
    cur = conn.cursor()
    if sub == "off":
        cur.execute("DELETE FROM daily_quiz_subs WHERE chat_id=?", (chat_id,))
        conn.commit()
        conn.close()
        await update.message.reply_text("☀️ Daily quiz band kar diya is group ke liye.")
        return
    if sub == "hour":
        if len(context.args) < 2 or not context.args[1].isdigit() or not (0 <= int(context.args[1]) <= 23):
            conn.close()
            await update.message.reply_text("Usage: `/dailyquiz hour <0-23>` (UTC hour)", parse_mode="Markdown")
            return
        cur.execute("UPDATE daily_quiz_subs SET hour_utc=? WHERE chat_id=?", (int(context.args[1]), chat_id))
        conn.commit()
        changed = cur.rowcount
        conn.close()
        if changed:
            await update.message.reply_text(f"☀️ Daily quiz ab {context.args[1]}:00 UTC par aayega.")
        else:
            await update.message.reply_text("☀️ Pehle `/dailyquiz on <topic>` se subscribe karo.")
        return
    if sub == "on":
        topic = " ".join(context.args[1:]).strip() or "General Knowledge"
        upsert(cur, "daily_quiz_subs", {"chat_id": chat_id},
               {"topic": topic[:100], "added_by": update.effective_user.id})
        conn.commit()
        conn.close()
        await update.message.reply_text(
            f"☀️ Daily quiz ON — topic: **{topic}**, roz 9:00 UTC par (badlo `/dailyquiz hour <0-23>` se).",
            parse_mode="Markdown",
        )
        return
    conn.close()
    await update.message.reply_text("Usage: `/dailyquiz on <topic>` / `off` / `hour <0-23>`", parse_mode="Markdown")


async def daily_quiz_check_job(context: ContextTypes.DEFAULT_TYPE):
    """Runs every hour; posts to any subscribed group whose chosen hour matches
    right now (UTC) and hasn't already been sent today."""
    now = datetime.utcnow()
    today = now.strftime("%Y-%m-%d")
    conn = db_connect()
    cur = conn.cursor()
    cur.execute("SELECT chat_id, topic, added_by FROM daily_quiz_subs WHERE hour_utc=? AND "
                "(last_sent_date IS NULL OR last_sent_date != ?)", (now.hour, today))
    due = cur.fetchall()
    conn.close()
    for chat_id, topic, added_by in due:
        try:
            questions = await generate_mcqs_async(added_by or OWNER_ID, [{"type": "text", "text": topic}],
                                                    10, "medium", "English")
            quiz_id = save_quiz(added_by or OWNER_ID, f"Quiz of the Day: {topic}"[:60], questions,
                                 timer_seconds=20, quiz_type="daily", creator_name="Daily Quiz")
            await context.bot.send_message(chat_id, f"☀️ **Quiz of the Day** — {topic}", parse_mode="Markdown")
            await initiate_live_quiz(context.bot, context.application, chat_id, "group", quiz_id,
                                      added_by or OWNER_ID, "Quiz of the Day")
        except Exception as e:
            logger.warning(f"Daily quiz failed for chat {chat_id}: {e}")
            continue
        conn = db_connect()
        cur = conn.cursor()
        cur.execute("UPDATE daily_quiz_subs SET last_sent_date=? WHERE chat_id=?", (today, chat_id))
        conn.commit()
        conn.close()


# ==========================================
# 🤖 AI MODE — owner switch that, once ON, auto-generates AI quiz polls spanning
# every subject from school up to graduate level and blasts them as native
# (non-anonymous) quiz polls straight into EVERY tracked/synced group, then
# auto-posts a full leaderboard (every participant's id + name, no matter how many)
# to every group once a day. Scoring here is its own rule, separate from the rest
# of the bot: ✅ correct = +5, ❌ wrong = -6 (AI_MODE_CORRECT_POINTS/WRONG_POINTS).
#
# Deliberately built as its own small, DB-backed system (its own 2 tables — see
# init_db — plus a couple of lines added to the existing shared PollAnswerHandler)
# instead of reusing the lobby-based live-quiz engine above: that engine waits for
# 2 "I Am Ready" taps per group before firing a single question, which is the
# opposite of what an automatic, no-friction, every-group broadcast needs.
# ==========================================
AI_MODE_CORRECT_POINTS = 5
AI_MODE_WRONG_POINTS = -2
AI_MODE_DEFAULT_INTERVAL_SECONDS = 24 * 3600
AI_MODE_DEFAULT_BATCH_SIZE = 5
AI_MODE_DEFAULT_LEADERBOARD_HOUR = 20  # IST (was UTC)


def _fmt_secs(sec: int) -> str:
    """Human-friendly display for an interval stored in seconds."""
    sec = int(sec)
    if sec < 60:
        return f"{sec}s"
    if sec < 3600:
...
def _ist_hour_to_utc_hour(ist_hour: int) -> int:
    """Floors an IST wall-clock hour (0-23) to the UTC hour it falls in — IST is
    UTC+5:30, so this stays a whole-hour value for ai_mode_leaderboard_job's
    once-per-hour check, firing within the first ~30 min of the intended IST hour."""
    return ((int(ist_hour) * 60 - 330) % 1440) // 60

# One of these is picked at random every cycle — spans school-board subjects up to
# graduate/undergraduate ones, so over many cycles the full "school se graduate tak"
# range actually gets covered rather than any single subject dominating. Edit this
# list directly to add, remove, or re-weight (repeat an entry to weight it higher).
AI_MODE_SUBJECTS = [
    "Mathematics (school level)", "Physics (school level)", "Chemistry (school level)",
    "Biology (school level)", "History", "Geography", "Political Science & Civics",
    "Economics", "English Language & Literature", "Computer Science", "Environmental Science",
    "General Knowledge", "Current Affairs", "Logical Reasoning & Aptitude",
    "Indian Constitution & Polity", "World History", "Static GK",
    "Engineering Mathematics (graduate level)", "Data Structures & Algorithms (graduate level)",
    "Database Management Systems (graduate level)", "Operating Systems (graduate level)",
    "Computer Networks (graduate level)", "Organic Chemistry (graduate level)",
    "Human Physiology (graduate level)", "Microeconomics (graduate level)",
    "Macroeconomics (graduate level)", "Business Studies & Management (graduate level)",
    "Statistics (graduate level)", "Accountancy & Financial Accounting (graduate level)",
    "Psychology (graduate level)", "Sociology (graduate level)",
]


def get_ai_mode_settings() -> dict:
    return {
        "enabled": get_bot_setting("ai_mode_enabled", "0") == "1",
        "interval_seconds": int(get_bot_setting("ai_mode_interval_seconds", "") or AI_MODE_DEFAULT_INTERVAL_SECONDS),
        "batch_size": int(get_bot_setting("ai_mode_batch_size", "") or AI_MODE_DEFAULT_BATCH_SIZE),
        "leaderboard_hour": int(get_bot_setting("ai_mode_leaderboard_hour", "") or AI_MODE_DEFAULT_LEADERBOARD_HOUR),
    }


async def aimode_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if update.effective_user.id != OWNER_ID:
        await update.message.reply_text("⛔ Owner only.")
        return
    if update.effective_chat.type != "private":
        await update.message.reply_text("🤖 AI Mode settings sirf bot ki DM me hi set/change ho sakti hain.")
        return
    s = get_ai_mode_settings()
    if not context.args:
        ...
        await update.message.reply_text(
            f"{ui_header('AI Mode', '🤖')}\n"
            f"{ui_status(state.split(' ',1)[1], ok=s['enabled'])}\n\n"
            f"• Har {_fmt_secs(s['interval_seconds'])} me {s['batch_size']} naye AI polls **per "
            f"language** — Hindi + English dono (Class 1 se Graduation/competitive-exam tak, hard "
            f"level) — {group_count} synced group me (~{s['batch_size'] * 2} polls/group/cycle)\n"
            f"• Daily leaderboard {s['leaderboard_hour']}:00 IST par har group me (sabhi participants — "
            f"jitne bhi ho, sabka id + name)\n"
            f"• Scoring: ✅ +{AI_MODE_CORRECT_POINTS} · ❌ {AI_MODE_WRONG_POINTS}\n\n"
            "Usage:\n"
            "`/aimode on [interval_seconds] [questions_per_batch_per_language]` — on karo (default "
            f"{_fmt_secs(AI_MODE_DEFAULT_INTERVAL_SECONDS)}, {AI_MODE_DEFAULT_BATCH_SIZE} Q)\n"
            "`/aimode off` — band karo\n"
            "`/aimode hour <0-23>` — leaderboard IST hour set karo",
            parse_mode="Markdown",
        )
        return
    sub = context.args[0].lower()
    if sub == "off":
        set_bot_setting("ai_mode_enabled", "0")
        await update.message.reply_text("🤖 AI Mode band kar diya — naye polls/leaderboard rukk jayenge.")
        return
    if sub == "on":
        interval = AI_MODE_DEFAULT_INTERVAL_SECONDS
        batch = AI_MODE_DEFAULT_BATCH_SIZE
        if len(context.args) >= 2:
            try:
                interval = max(1, int(context.args[1]))
            except ValueError:
                await update.message.reply_text("❌ interval_seconds ek number hona chahiye.")
                return
        if len(context.args) >= 3:
            try:
                batch = max(1, min(20, int(context.args[2])))
            except ValueError:
                await update.message.reply_text("❌ questions_per_batch ek number hona chahiye.")
                return
        set_bot_setting("ai_mode_enabled", "1")
        set_bot_setting("ai_mode_interval_seconds", str(interval))
        set_bot_setting("ai_mode_batch_size", str(batch))
        set_bot_setting("ai_mode_last_sent_at", "")  # first batch fires on the very next check
        conn = db_connect(); cur = conn.cursor()
        cur.execute("SELECT COUNT(*) FROM groups")
        group_count = cur.fetchone()[0]
        conn.close()
        warn = (
            "\n⚠️ Itna chhota interval AI-generation + Telegram rate-limit ki apni speed se bandha "
            "hai — har group ko batch bhejne me jitna time lagega, agla cycle uske baad hi shuru "
            "hoga, literally is second nahi." if interval < 30 else ""
        )
        await update.message.reply_text(
            f"🤖 **AI Mode ON!**\n\n"
            f"• Har {_fmt_secs(interval)} me {batch} naye AI-generated polls **per language** "
            f"(Hindi + English, hard level, Class 1–Graduation/exam mix) — {group_count} synced "
            f"group me jaayenge (~{batch * 2} polls/group/cycle, pehla batch ~5 sec ke andar){warn}\n"
            f"• Roz {get_ai_mode_settings()['leaderboard_hour']}:00 IST par har group me automatic "
            f"leaderboard — jitne bhi users ne answer diya, sabka id + name\n"
            f"• Scoring: ek sahi par +{AI_MODE_CORRECT_POINTS}, galat par {AI_MODE_WRONG_POINTS}\n\n"
            "`/aimode hour <0-23>` se leaderboard ka IST time badal sakte ho.",
            parse_mode="Markdown",
        )
        return
    if sub == "hour":
        if len(context.args) < 2 or not context.args[1].isdigit() or not (0 <= int(context.args[1]) <= 23):
            await update.message.reply_text("Usage: `/aimode hour <0-23>` (IST)", parse_mode="Markdown")
            return
        set_bot_setting("ai_mode_leaderboard_hour", context.args[1])
        await update.message.reply_text(
            f"🤖 AI Mode leaderboard ab roz {context.args[1]}:00 IST (lagbhag) par post hoga."
        )
        return
    await update.message.reply_text(
        "Usage: `/aimode on [interval_seconds] [questions_per_batch]` / `off` / `hour <0-23>`",
        parse_mode="Markdown",
    )


def _prune_old_ai_mode_polls():
    cutoff = (datetime.utcnow() - timedelta(days=7)).strftime("%Y-%m-%d %H:%M:%S")
    conn = db_connect()
    cur = conn.cursor()
    cur.execute("DELETE FROM ai_mode_polls WHERE created_at < ?", (cutoff,))
    conn.commit()
    conn.close()


async def ai_mode_quiz_check_job(context: ContextTypes.DEFAULT_TYPE):
    """Runs every ~5 sec. When AI Mode is ON and `interval_seconds` has elapsed since
    the last batch, generates a fresh AI quiz on ONE randomly-picked subject (see
    AI_MODE_SUBJECTS), in BOTH English and Hindi, and fires it as native quiz polls
    into every tracked group — no lobby, no "ready" wait, straight broadcast (like
    /quizblast but automatic, bilingual, AI-written, and recurring)."""
    if get_bot_setting("ai_mode_enabled", "0") != "1":
        return
    s = get_ai_mode_settings()
    now = datetime.utcnow()
    last_sent = get_bot_setting("ai_mode_last_sent_at", "")
    if last_sent:
        try:
            if now - datetime.fromisoformat(last_sent) < timedelta(seconds=s["interval_seconds"]):
                return
        except ValueError:
            pass

    subject = random.choice(AI_MODE_SUBJECTS)
    difficulty = (
        "hard difficulty, school-to-graduate level (mix of school-board, undergraduate, "
        "graduate and competitive-exam difficulty)"
    )
    questions = []
    for lang in ("English", "Hindi"):
        try:
            questions.extend(await generate_mcqs_async(
                OWNER_ID, [{"type": "text", "text": subject}], s["batch_size"], difficulty, lang,
            ))
        except Exception as e:
            logger.warning(f"AI Mode generation failed for subject '{subject}' ({lang}): {e}")
    if not questions:
        return

    # Mark "sent" now, before per-group delivery — so a few unreachable/kicked-from
    # groups below can never stall every future cycle retrying the same batch.
    set_bot_setting("ai_mode_last_sent_at", now.isoformat())
    try:
        save_quiz(OWNER_ID, f"AI Mode: {subject}"[:60], questions, quiz_type="ai_mode", creator_name="AI Mode")
    except Exception as e:
        logger.warning(f"AI Mode save_quiz failed (non-fatal, polls still send): {e}")
    ...
    for gid in group_ids:
        try:
            await context.bot.send_message(
                gid, f"🤖 **AI Mode Quiz** — {subject} (English + हिन्दी)", parse_mode="Markdown"
            )
        except Exception as e:
            logger.warning(f"AI Mode intro message failed for group {gid} (skipped, others unaffected): {e}")
            continue
        for q in questions:
            try:
                msg = await context.bot.send_poll(
                    chat_id=gid, question=q["question"][:300],
                    options=[o[:100] for o in q["options"]],
                    type=Poll.QUIZ, correct_option_id=q["correct_index"],
                    explanation=(q.get("explanation", "")[:200] or None),
                    is_anonymous=False,
                )
                conn = db_connect()
                cur = conn.cursor()
                cur.execute(
                    "INSERT INTO ai_mode_polls (poll_id, chat_id, correct_option_id, subject) VALUES (?, ?, ?, ?)",
                    (msg.poll.id, gid, q["correct_index"], subject),
                )
                conn.commit()
                conn.close()
            except Exception as e:
                logger.warning(f"AI Mode poll send failed for group {gid} (skipped, others unaffected): {e}")
            await asyncio.sleep(0.05)
        sent_groups += 1
    logger.info(f"AI Mode: sent {len(questions)} poll(s) on '{subject}' to {sent_groups}/{len(group_ids)} group(s).")
    _prune_old_ai_mode_polls()


def record_ai_mode_answer(chat_id: int, user_id: int, user_name: str, correct: bool):
    today = datetime.utcnow().strftime("%Y-%m-%d")
    delta = AI_MODE_CORRECT_POINTS if correct else AI_MODE_WRONG_POINTS
    conn = db_connect()
    cur = conn.cursor()
    cur.execute(
        "SELECT score, correct_count, wrong_count FROM ai_mode_scores WHERE chat_id=? AND user_id=? AND score_date=?",
        (chat_id, user_id, today),
    )
    row = cur.fetchone()
    if row:
        cur.execute(
            "UPDATE ai_mode_scores SET score=?, correct_count=?, wrong_count=?, user_name=? "
            "WHERE chat_id=? AND user_id=? AND score_date=?",
            (row[0] + delta, row[1] + (1 if correct else 0), row[2] + (0 if correct else 1),
             user_name[:60], chat_id, user_id, today),
        )
    else:
        cur.execute(
            "INSERT INTO ai_mode_scores (chat_id, user_id, user_name, score_date, score, correct_count, wrong_count) "
            "VALUES (?, ?, ?, ?, ?, ?, ?)",
            (chat_id, user_id, user_name[:60], today, delta, 1 if correct else 0, 0 if correct else 1),
        )
    conn.commit()
    conn.close()


def build_ai_mode_leaderboard_chunks(chat_id: int, score_date: str):
    """Every participant for that group+day — id + name + score, sorted best-first —
    deliberately NOT capped at a top-N like /leaderboard, since AI Mode's whole point
    is showing everyone who played, chahe kitne bhi ho. Returns None if nobody
    answered that day; otherwise a list of message chunks kept under Telegram's
    ~4096-char limit, so a very active group with hundreds of participants still
    gets a complete list — split across a few messages instead of dropping anyone."""
    conn = db_connect()
    cur = conn.cursor()
    cur.execute(
        "SELECT user_id, user_name, score, correct_count, wrong_count FROM ai_mode_scores "
        "WHERE chat_id=? AND score_date=? ORDER BY score DESC",
        (chat_id, score_date),
    )
    rows = cur.fetchall()
    conn.close()
    ...
    medals = ["🥇", "🥈", "🥉"]
    header = (
        f"{ui_header('Daily Leaderboard', '🏆')}\n"
        f"📅 {score_date} · ✅ +{AI_MODE_CORRECT_POINTS} · ❌ {AI_MODE_WRONG_POINTS} · "
        f"👥 {len(rows)} participant(s)\n\n"
    )
    chunks, current = [], header
    for i, (user_id, name, score, correct, wrong) in enumerate(rows):
        rank_tag = medals[i] if i < 3 else f"{i + 1}."
        line = f"{rank_tag} {name or 'Player'} — `{user_id}` — {score} pts (✅{correct} ❌{wrong})\n"
...
async def ai_mode_leaderboard_job(context: ContextTypes.DEFAULT_TYPE):
    """Runs hourly; once a day, at the configured IST hour (converted to UTC via
    _ist_hour_to_utc_hour), posts the full AI Mode leaderboard (every participant —
    see build_ai_mode_leaderboard_chunks) to every tracked group. Same 'checked
    often, guarded by a last-posted-date' pattern as daily_quiz_check_job above, so
    it fires exactly once per UTC day."""
    if get_bot_setting("ai_mode_enabled", "0") != "1":
        return
    s = get_ai_mode_settings()
    now = datetime.utcnow()
    if now.hour != _ist_hour_to_utc_hour(s["leaderboard_hour"]):
        return
    today = now.strftime("%Y-%m-%d")
    if get_bot_setting("ai_mode_last_leaderboard_date", "") == today:
        return
    ...
    posted = 0
    for gid in group_ids:
        chunks = build_ai_mode_leaderboard_chunks(gid, today)
        if not chunks:
            continue
        try:
            for chunk in chunks:
                await context.bot.send_message(gid, chunk, parse_mode="Markdown")
                await asyncio.sleep(0.1)
            posted += 1
        except Exception as e:
            logger.warning(f"AI Mode leaderboard post failed for group {gid} (skipped, others unaffected): {e}")
        await asyncio.sleep(0.05)
    set_bot_setting("ai_mode_last_leaderboard_date", today)
    logger.info(f"AI Mode daily leaderboard posted to {posted}/{len(group_ids)} group(s) for {today}.")


# ==========================================
# 📢 /schedulebroadcast — owner sends a plain-text message to every user/group at a
# chosen future UTC time (durable across restarts — stored in DB, checked every minute).
# ==========================================
async def schedulebroadcast_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if len(context.args) < 3:
        await update.message.reply_text(
            "Usage: `/schedulebroadcast YYYY-MM-DD HH:MM <message>` (UTC time)\n"
            "Example: `/schedulebroadcast 2026-09-01 09:00 New feature is live!`",
            parse_mode="Markdown",
        )
        return
    ...
    try:
        ...
        await update.message.reply_text("❌ Date/time format samajh nahi aaya — `YYYY-MM-DD HH:MM` use karo (UTC).")
        return
    if run_at <= datetime.utcnow():
        await update.message.reply_text("❌ Ye time already beet chuka hai (UTC) — future time do.")
        return
    conn = db_connect()
    cur = conn.cursor()
    cur.execute("INSERT INTO scheduled_broadcasts (message, run_at, created_by) VALUES (?, ?, ?)",
                (message, run_at.isoformat(), update.effective_user.id))
    conn.commit()
    conn.close()
    await update.message.reply_text(f"📢 Scheduled for {run_at.isoformat()} UTC:\n\n{message}")


async def scheduled_broadcast_check_job(context: ContextTypes.DEFAULT_TYPE):
    now_iso = datetime.utcnow().isoformat()
    conn = db_connect()
    cur = conn.cursor()
    cur.execute("SELECT id, message FROM scheduled_broadcasts WHERE sent=0 AND run_at<=?", (now_iso,))
    due = cur.fetchall()
    conn.close()
    for bid, message in due:
        try:
            ok, fail = await broadcast_to_all(context.bot, message)
            logger.info(f"Scheduled broadcast #{bid} sent: {ok} ok, {fail} failed.")
        except Exception as e:
            logger.warning(f"Scheduled broadcast #{bid} failed: {e}")
        conn = db_connect()
        cur = conn.cursor()
        cur.execute("UPDATE scheduled_broadcasts SET sent=1 WHERE id=?", (bid,))
        conn.commit()
        conn.close()


# ==========================================
# 💬 /feedback — lets any user message the owner without needing their contact info
# ==========================================
async def feedback_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not context.args:
        await update.message.reply_text("Usage: `/feedback <your message>` — seedha bot owner ko chala jayega.", parse_mode="Markdown")
        return
    message = " ".join(context.args)[:1000]
    user = update.effective_user
    conn = db_connect()
    cur = conn.cursor()
    cur.execute("INSERT INTO feedback (user_id, user_tag, message) VALUES (?, ?, ?)",
                (user.id, user.username or user.first_name or "Unknown", message))
    conn.commit()
    conn.close()
    await update.message.reply_text("✅ Bhej diya — dhanyavaad! Owner tak pahunch gaya.")
    if OWNER_ID:
        try:
            await context.bot.send_message(
                OWNER_ID,
                f"💬 **Feedback from** {user.first_name} (`{user.id}`, @{user.username or 'no_username'}):\n\n{message}",
                parse_mode="Markdown",
            )
        except Exception as e:
            logger.warning(f"Failed to forward feedback to owner: {e}")
...
async def restorebackup_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        lines = "\n".join(f"• {t}: {n if isinstance(n, str) else f'{n} row(s)'}" for t, n in summary.items())
...
def run_migrations():
    ...
    new_columns = (
        ...
        ("users", "poll_style", "TEXT DEFAULT 'native'"),
        ...
        ("scheduled_quizzes", "created_by", "BIGINT"),
        ...
        ("users", "last_quiz_date", "TEXT DEFAULT NULL"),
        ("users", "streak_freeze_count", "INTEGER DEFAULT 0"),
        ("users", "referral_milestone_claimed", "INTEGER DEFAULT 0"),
        ("users", "coins", "INTEGER DEFAULT 100"),
        ("users", "best_solo_score", "TEXT DEFAULT '{}'"),
        ("duels", "wager", "INTEGER DEFAULT 0"),
    )
    ...
    # A DB created before this fix has these columns as 32-bit INTEGER on Postgres
    # (fine on SQLite, which never enforced a width) — widen them in place so
    # already-deployed Supabase/Render Postgres databases stop rejecting today's
    # Telegram IDs, which now regularly exceed the 32-bit ~2.1B ceiling.
    if USE_POSTGRES:
        bigint_columns = [
            ("users", "user_id"), ("users", "referred_by"), ("filters", "group_id"),
            ("quizzes", "user_id"), ("groups", "group_id"), ("quiz_results", "chat_id"),
            ("quiz_results", "user_id"), ("group_settings", "group_id"), ("quiz_sets", "owner_id"),
            ("user_channels", "user_id"), ("user_channels", "channel_id"), ("redeem_uses", "user_id"),
            ("last_source", "user_id"), ("user_api_keys", "user_id"), ("scheduled_quizzes", "user_id"),
            ("scheduled_quizzes", "chat_id"), ("scheduled_quizzes", "created_by"), ("duels", "chat_id"),
            ("duels", "challenger_id"), ("duels", "opponent_id"), ("daily_quiz_subs", "chat_id"),
            ("daily_quiz_subs", "added_by"), ("feedback", "user_id"), ("scheduled_broadcasts", "created_by"),
        ]
        for table, col in bigint_columns:
            try:
                ...
                logger.warning(f"BIGINT migration skipped for {table}.{col}: {e}")
                conn.rollback()
...
def sync_serial_sequences():
    """Bumps every SERIAL-PK sequence to max(id)+1. Needed once after any restore
    that inserted rows with explicit id values (see restore_from_backup) — safe to
    run on every startup regardless, since it's a no-op when the sequence is
    already ahead of max(id)."""
    if not USE_POSTGRES:
        return
    conn = db_connect()
    try:
        for table in ("quizzes", "quiz_results", "user_channels", "scheduled_quizzes",
                       "duels", "feedback", "scheduled_broadcasts", "user_api_keys", "quiz_sets",
                       "advance_stickers", "wrong_answers", "tournaments", "time_capsules"):
            try:
                cur = conn.cursor()
                cur.execute(
...
# init_db()/run_migrations() used to run here at import time — outside
# run_forever()'s retry loop. Any transient DB hiccup on startup (Supabase's
# pooler briefly refusing a connection, a DNS blip, etc.) killed the whole
# process instantly instead of getting the same backoff-and-retry every other
# failure gets, which is what caused the crash-restart cycles / not staying up
# 24x7. They now run inside main() (see below), guarded by run_forever().
...
def get_user(user_id: int) -> dict:
    ...
    cursor.execute(
        ...
        "daily_ai_date, ai_unlimited, option_style, poll_style, coins, best_solo_score "
    ...
    if not user:
        cursor.execute(
            "INSERT INTO users (user_id, questions_used, question_limit, is_verified) VALUES (?, 0, 999999, 0)",
            (user_id,)
        )
        conn.commit()
        cursor.execute(
    ...
    return {
        ...
        "poll_style": user[12] or 'native',
        "coins": user[13] if user[13] is not None else 100,
        "best_solo_score": user[14] or '{}',
    }
...
def set_user_field(user_id: int, field: str, value):
    allowed = {"tag", "description_tag", "default_difficulty", "default_language", "referred_by", "option_style", "poll_style"}
...
# Set to a very high number so users never hit the limit.
# Owner and /freeuser-granted users are always truly unlimited.
# ==========================================
DAILY_AI_QUESTION_LIMIT = 999999
...
POLL_STYLE_LABELS = {"native": "🤖 Native (plain, official-quiz-bot style)", "pro": "💎 Pro (stylish + always shows explanation)"}


def apply_poll_style(question_text: str, explanation: str, poll_style: str):
    """Native = untouched, exactly like a plain official-quiz-bot poll (current
    default behaviour, unchanged). Pro = a distinct visual wrapper on the question
    text, and guarantees an explanation is always present (auto-filling a generic
    one if the question didn't have one) so the "why" always shows to the answerer
    on a wrong pick. Telegram's poll UI already renders the per-option vote/percentage
    breakdown natively once a poll is answered/closed — that's a client-side feature
    of every non-anonymous quiz poll and needs no extra work here to "show a graph"."""
    if poll_style != 'pro':
        return question_text, explanation
    styled_question = f"💎 ┃ {question_text}"
    if not (explanation or '').strip():
        explanation = "💎 Pro poll — check the highlighted correct option above."
    else:
        explanation = f"💎 {explanation}"
    return styled_question, explanation


def _poll_style_keyboard(current: str):
    rows = []
    for key, label in POLL_STYLE_LABELS.items():
        prefix = "✅ " if key == current else ""
        rows.append([InlineKeyboardButton(f"{prefix}{label}", callback_data=f"pollstyle_{key}")])
    rows.append([InlineKeyboardButton("🔙 Back", callback_data="tools_menu")])
    return InlineKeyboardMarkup(rows)


async def poll_style_settings_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    await query.answer()
    user = get_user(update.effective_user.id)
    await query.edit_message_text(
        f"{ui_header('Poll Style', '💎')}\nChoose how your polls look everywhere they're posted "
        "(quiz store, live quizzes, AI Mode, official quiz):",
        parse_mode="Markdown", reply_markup=_poll_style_keyboard(user['poll_style']),
    )


async def poll_style_set_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    await query.answer()
    style = query.data.replace("pollstyle_", "")
    if style not in POLL_STYLE_LABELS:
        return
    set_user_field(update.effective_user.id, 'poll_style', style)
    await query.edit_message_text(
        f"{ui_header('Poll Style', '💎')}\n✅ Set to: {POLL_STYLE_LABELS[style]}",
        parse_mode="Markdown", reply_markup=_poll_style_keyboard(style),
    )


# ==========================================
# 🕵️ KRISH MODE (/krish) — ping, a direct line to the owner, and the "Advance"
# sticker game.
# ==========================================

# Best-effort filter only: Telegram's Bot API gives no way to inspect a sticker's
# actual image content, so this cannot reliably detect NSFW artwork inside a
# sticker that's packaged under an innocent-looking name. What it CAN catch is a
# sticker pack whose own set name/title openly signals adult content — good
# enough to block the obvious cases, but not a substitute for real moderation.
# If tighter filtering is needed later, that requires running the sticker's
# image through a vision/moderation model, which isn't wired up here.
ADVANCE_STICKER_BLOCKLIST = [
    "nsfw", "18+", "xxx", "porn", "sex", "nude", "naked", "hentai", "adult",
    "boobs", "hot_girl", "hotgirl",
]


def _is_sticker_blocked(sticker) -> bool:
    name = ((sticker.set_name or "") + " " + (sticker.emoji or "")).lower()
    return any(bad in name for bad in ADVANCE_STICKER_BLOCKLIST)


def save_advance_sticker(file_id: str, set_name: str, user_id: int):
    conn = db_connect()
    try:
        cur = conn.cursor()
        cur.execute(
            "INSERT INTO advance_stickers (file_id, set_name, added_by) VALUES (?, ?, ?)",
            (file_id, set_name or '', user_id),
        )
        conn.commit()
        return True
    except Exception:
        return False  # duplicate file_id (already in the pool) — not an error
    finally:
        conn.close()


def get_random_advance_sticker(exclude_file_id: str = None):
    conn = db_connect()
    try:
        cur = conn.cursor()
        if exclude_file_id:
            cur.execute(
                "SELECT file_id FROM advance_stickers WHERE file_id != ? ORDER BY RANDOM() LIMIT 1",
                (exclude_file_id,),
            )
        else:
            cur.execute("SELECT file_id FROM advance_stickers ORDER BY RANDOM() LIMIT 1")
        row = cur.fetchone()
...
def _krish_menu_text():
    return (
        f"{ui_header('Krish Mode', '🕵️')}\n"
        f"{ui_bullet('Ping — check the bot’s live response time', '🏓')}\n"
        f"{ui_bullet('Meet the Owner — send a message straight to the creator', '👤')}\n"
        f"{ui_bullet('Advance — the sticker game (send a sticker, get one back)', '🎮')}\n"
    )


def _krish_menu_keyboard():
    return InlineKeyboardMarkup([
        [InlineKeyboardButton("🏓 Ping", callback_data="krish_ping")],
        [InlineKeyboardButton("👤 Meet the Owner", callback_data="krish_meet_owner")],
        [InlineKeyboardButton("🎮 Advance (Sticker Game)", callback_data="krish_advance")],
        [InlineKeyboardButton("🔙 Back to Tools", callback_data="open_tools")],
    ])


async def krish_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text(_krish_menu_text(), parse_mode="Markdown", reply_markup=_krish_menu_keyboard())


async def krish_menu_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    await query.answer()
    await query.edit_message_text(_krish_menu_text(), parse_mode="Markdown", reply_markup=_krish_menu_keyboard())


async def krish_ping_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    start = time.monotonic()
    await query.answer("Pinging…")
    latency_ms = round((time.monotonic() - start) * 1000)
    # Also let the owner see it's being used — a light heartbeat, not a per-tap
    # spam: only mentions who pinged, no extra noise.
    if update.effective_user.id != OWNER_ID:
        try:
            await context.bot.send_message(
                OWNER_ID,
                f"🏓 Krish Mode ping from {update.effective_user.first_name} "
                f"(`{update.effective_user.id}`) — {latency_ms}ms",
                parse_mode="Markdown",
            )
        except Exception:
            pass
    await query.edit_message_text(
        f"{ui_header('Krish Mode', '🕵️')}\n🏓 Pong! **{latency_ms}ms**",
        parse_mode="Markdown", reply_markup=_krish_menu_keyboard(),
    )


async def krish_meet_owner_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    await query.answer()
    context.user_data['krish_awaiting_owner_msg'] = True
    await query.edit_message_text(
        f"{ui_header('Meet the Owner', '👤')}\nType your message now — it'll go straight to the owner's DM.",
        parse_mode="Markdown", reply_markup=back_to_tools_keyboard(),
    )


async def krish_receive_owner_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not context.user_data.get('krish_awaiting_owner_msg'):
        return  # not in this flow — leave the message for any other handler
    context.user_data['krish_awaiting_owner_msg'] = False
    u = update.effective_user
    try:
        await context.bot.send_message(
            OWNER_ID,
            f"👤 **Message from {u.first_name}** (`{u.id}`{f', @{u.username}' if u.username else ''}) "
            f"via Krish Mode:\n\n{update.message.text}",
            parse_mode="Markdown",
        )
        await update.message.reply_text("✅ Sent to the owner. They'll get back to you directly.")
    except Exception:
        await update.message.reply_text("⚠️ Couldn't deliver that right now — try again in a bit.")
    raise ApplicationHandlerStop


async def krish_advance_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    await query.answer()
    context.user_data['advance_game'] = True
    await query.edit_message_text(
        f"{ui_header('Advance', '🎮')}\n"
        "Sticker game is on! Send me a sticker and I'll send one right back — "
        "from everyone else who's played, so it feels like there's someone "
        "live on the other end. 18+/NSFW packs are filtered out and never saved.\n\n"
        "Send /krish anytime to come back here.",
        parse_mode="Markdown",
    )


async def handle_advance_sticker(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not context.user_data.get('advance_game'):
        return  # game not active for this user — ignore, let other handlers run
    sticker = update.message.sticker
    if _is_sticker_blocked(sticker):
        await update.message.reply_text("🚫 That pack isn't allowed here — try a different sticker.")
        raise ApplicationHandlerStop
    save_advance_sticker(sticker.file_id, sticker.set_name, update.effective_user.id)
    reply_id = get_random_advance_sticker(exclude_file_id=sticker.file_id)
    if reply_id:
        await update.message.reply_sticker(reply_id)
    else:
        await update.message.reply_text("👀 Got it — you're the first one in! Send more, or wait for others to join in.")
    raise ApplicationHandlerStop
...
def update_daily_streak(user_id: int) -> int:
    """Called once per finished quiz a user participates in. Bumps their streak by 1
    if today is the day right after their last counted day, resets to 1 if they broke
    the chain, and leaves it untouched if they already played today (so answering 5
    quizzes in one day doesn't inflate the count). Returns the resulting streak.

    Streak Freeze: if exactly one day was missed and the user is holding a freeze
    token (earned every 7-day milestone, capped at 3), one token is spent to keep
    the chain alive instead of resetting to 1."""
    today = datetime.utcnow().strftime("%Y-%m-%d")
    yesterday = (datetime.utcnow() - timedelta(days=1)).strftime("%Y-%m-%d")
    day_before = (datetime.utcnow() - timedelta(days=2)).strftime("%Y-%m-%d")
    conn = db_connect()
    cur = conn.cursor()
    cur.execute("SELECT streak_count, last_quiz_date, streak_freeze_count FROM users WHERE user_id=?", (user_id,))
    row = cur.fetchone()
    streak, last_date, freezes = (row[0] or 0, row[1], row[2] or 0) if row else (0, None, 0)
    if last_date == today:
        conn.close()
        return streak
    if last_date == yesterday:
        streak += 1
    elif last_date == day_before and freezes > 0:
        streak += 1
        freezes -= 1
    else:
        streak = 1
    if streak > 0 and streak % 7 == 0 and freezes < 3:
        freezes += 1  # milestone reward — a fresh freeze token, capped at 3 held
    cur.execute(
        "UPDATE users SET streak_count=?, last_quiz_date=?, streak_freeze_count=? WHERE user_id=?",
...
def build_provider_chain(user_id: int) -> list:
    ...
    first (in the order they added them with /addapi), then EVERY owner-level
    key the bot owner has configured via env vars (OWNER_PROVIDER_KEYS) — including
    multiple comma-separated keys for the same provider, each treated as its own
    fallback entry with its own usage tracking — tried in OWNER_FALLBACK_ORDER,
    with Anthropic last. This keeps generation working ("meri wali to run kregi")
    no matter how many keys a user has added (or whether they've added any at
    all), and no matter which of the owner's own keys happen to be rate-limited
    at that moment.
    """
    chain = []
    ...
    for provider in OWNER_FALLBACK_ORDER:
        ...
        for idx, api_key in enumerate(api_keys):
            ...
            if preset.get("needs_base"):
                # Cloudflare-style providers need an account-specific base URL built
                # from a second env var rather than a fixed preset base. Multiple keys
                # paired with fewer account IDs reuse the last account ID given.
                if not account_ids:
                    continue  # key(s) without any account id are unusable — skip silently
...
# rok sake ya users ko warn kar sake.
#
# ⚠️ Ye defaults har provider ke PUBLISHED FREE-TIER daily request cap par based hain
# (verified ~August 2026) — lekin har provider apni free-tier limits BINA notice ke
# badal sakta hai. Har ek `<PROVIDER>_DAILY_LIMIT` env var se override ho sakta hai
# agar tumhara actual plan/limit alag hai (jaise Groq Developer tier, ya OpenRouter
# jisne $10 credit purchase kiya ho). Bot ka apna counter hamesha UTC midnight par
# reset hota hai (chaahe provider ka real reset kisi bhi cheez par ho — daily UTC,
# rolling window, ya monthly credit) — ye ek consistent internal baseline hai, provider
# ke asli reset time ka exact substitute nahi.
# ==========================================
DEFAULT_KEY_DAILY_LIMIT = int(os.getenv("KEY_DAILY_LIMIT", "1500"))
USAGE_ALERT_THRESHOLDS = [50, 70, 90, 100]

# Per-provider default daily request caps (override any of these with an env var,
# e.g. GROQ_DAILY_LIMIT=500). None = don't track (custom/self-hosted, or a provider
# whose free tier isn't a clean daily-request number).
PROVIDER_DAILY_LIMIT_DEFAULTS = {
    "groq": 1000,               # llama-3.3-70b-versatile free tier: ~1,000 RPD (resets ~UTC)
    "gemini": 1500,             # Flash models free tier: 1,500 RPD (resets ~midnight Pacific)
    "openai": None,             # not a real "free" daily cap — tied to account spend tier
    "anthropic": None,          # owner's paid key — org-specific tier, no generic public number
    "github": 150,              # gpt-4o-mini on GitHub Models: 150 RPD (per-model, ~24h rolling)
    "openrouter_vision": 50,    # :free models: 50 RPD (no credit purchased) / 1,000 RPD after $10 credit
    "openrouter_text": 50,      # same free-tier cap as above — bump via env var if you've bought credit
    "cloudflare_vision": 300,   # 10,000 Neurons/day shared pool; vision burns more Neurons/call
    "cloudflare_text": 1000,    # same 10,000-Neuron pool; small text model burns far fewer/call
    "mistral": 500,             # Mistral doesn't publish an exact free RPD — rough safety estimate
    "cerebras": 14000,          # ~14,400 RPD (buffer kept) — 1M tokens/day may bind first on long prompts
    "huggingface": 10,          # Inference Providers free credit is <$0.10/MONTH — practically a handful of calls
}


def get_provider_daily_limit(provider: str):
    """None means 'don't track/limit this provider'. Otherwise checks
    PROVIDER_DAILY_LIMIT_DEFAULTS first, then falls back to DEFAULT_KEY_DAILY_LIMIT for
    anything not listed there — always overridable via a `<PROVIDER>_DAILY_LIMIT` env var."""
    if provider == "custom":
        return None
...
def tools_menu_keyboard():
    keyboard = [
        ...
        [
            InlineKeyboardButton("🕵️ Krish Mode", callback_data="krish_menu")
        ],
        [
...
def build_welcome_text(user_name: str, user_data: dict, verified_banner: bool = False) -> str:
    if verified_banner:
        icon, title = "✅", "Access Granted"
    else:
        icon, title = "✨", f"Welcome, {user_name}"
    lines = [
        ui_header(title, icon),
        "",
        ui_row("User ID", user_data['user_id']),
        ui_row("Quota", f"{user_data['questions_used']}/{user_data['question_limit']}"),
        ui_row("Status", "🟢 Verified"),
        "",
        ui_bullet("Tap **🛠️ Tools** below to unlock quiz creation & group features."),
        "",
        ui_footer(),
    ]
    return "\n".join(lines)
...
def _build_system_prompt(count: int, difficulty: str, language: str) -> str:
    marwari_note = ""
    if language == "Hindi":
        # Marwari/Rajasthani flavor for Hindi output, per the user's reference sheet —
        # common everyday-speech substitutions, not a full dialect rewrite of every word.
        marwari_note = (
            "\n\nSTYLE (Hindi output specifically): flavor the Hindi with everyday "
            "Marwari/Rajasthani vocabulary and pronunciation where it fits naturally — "
            "e.g. कांई for क्या, थारो/म्हारो for तुम्हारा/मेरा, घणो for बहुत/ज्यादा, "
            "टाबर for बच्चा, चोखो for अच्छा, पाछो for वापस, नूवो for नया, आपां for हम, "
            "बकील/बिचार style 'व'→'ब', 'यश'→'जस' style 'य'→'ज'. Keep it readable and "
            "natural, not forced — technical/subject terms (science, math, proper nouns) "
            "should stay standard Hindi so the question remains factually clear; apply the "
            "Marwari flavor to the connecting/everyday words around them."
        )
    return (
        ...
        f"languages within the same item (e.g. Hindi question with English options is NOT "
        f"acceptable unless {language} literally is a mix like Hinglish). Proper nouns "
        f"(names, places) may stay as commonly written, but every other word must be in "
        f"{language}.{marwari_note}\n\n"
...
GEN_BATCH_SIZE = 20  # generate in chunks this size — keeps each single AI response
                      # short enough to reliably finish (this is the main fix for
                      # "utne question nahi banata": asking one huge model call for
                      # 50-100 questions in one JSON blob commonly gets truncated or
                      # cut short by the provider's own output-token limit; asking
                      # for 20 at a time and stitching the batches together doesn't).


def _generate_batch(user_id: int, source_parts: list, batch_count: int, difficulty: str, language: str) -> list:
    """Runs the full provider fallback chain for ONE batch (<=GEN_BATCH_SIZE
    questions). This is exactly the original single-shot generate_mcqs_sync body —
    generate_mcqs_sync below just calls this once per batch and stitches the
    results together so a big request (50/75/100) doesn't ride on a single,
    easily-truncated AI response."""
...
def generate_mcqs_sync(user_id: int, source_parts: list, count: int, difficulty: str, language: str) -> list:
    """
    Synchronous, SINGLE-THREAD version — runs every batch one after another. Only
    used where a plain blocking call is fine (small known counts, e.g. Voice → Quiz).
    For anything driven by the /topic /pdf /image /section flow, use
    generate_mcqs_async below instead — it runs batches concurrently so one user's
    big (50-100 question) request doesn't tie up a single thread for a long time
    and starve everyone else's polls/commands while it's running.
    """
...
async def generate_mcqs_async(user_id: int, source_parts: list, count: int, difficulty: str, language: str) -> list:
    """
    Preferred entry point for the AI-generation handlers. Splits `count` into
    GEN_BATCH_SIZE-sized batches and runs them CONCURRENTLY (each in its own
    worker thread via asyncio.to_thread + asyncio.gather) instead of one after
    another. This is the fix for "bot ek jagah AI chalata hai to dusri jagah
    commands/polls me time lagta hai": a single big request used to occupy one
    thread for a long, serial stretch (batch 1 done, THEN batch 2, THEN batch
    3...) — now all its batches run in parallel, so the request finishes in
    roughly the time of ONE batch instead of N, freeing up threads for other
    users' commands much sooner. See main() for the enlarged thread pool this
    relies on to actually run many of these concurrently across different users.

    A batch that fails outright raises; but if at least one batch succeeded
    before that, the partial result is still returned rather than discarding
    everything already generated (better to hand back 62/75 questions than none).
    """
    count = max(1, min(count, 100))
    batch_sizes = []
    remaining = count
...
async def post_quiz_polls(bot, chat_id, questions: list, user_tag: str = '', explanation_tag: str = '', option_style: str = 'plain', poll_style: str = 'native') -> int:
    sent = 0
    for q in questions:
        ...
        question_text, explanation = apply_poll_style(question_text, explanation, poll_style)
        question_text = question_text[:300]
...
async def require_verification(update: Update, context: ContextTypes.DEFAULT_TYPE) -> bool:
    ...
    if msg:
        await msg.reply_text(
            text=(
                f"{ui_header('Verification Required', '🔐')}\n\n"
...
# 🎁 REFERRAL REWARDS
# ==========================================
REFERRAL_MILESTONES = [(50, 500), (25, 200), (10, 50), (5, 20)]  # (referrals needed, bonus questions)
...
async def _maybe_grant_referral_milestone(bot, referrer_id: int):
    """Every 5/10/25/50 successful referrals unlocks a one-time bonus on top of the
    existing +10-questions-per-referral reward. Each milestone is only ever granted
    once per user (tracked via referral_milestone_claimed, the highest milestone
    count already paid out)."""
    total = count_referrals(referrer_id)
    user = get_user(referrer_id)
    conn = db_connect()
    cur = conn.cursor()
    cur.execute("SELECT referral_milestone_claimed FROM users WHERE user_id=?", (referrer_id,))
    row = cur.fetchone()
    claimed = (row[0] or 0) if row else 0
    conn.close()
    for needed, bonus in sorted(REFERRAL_MILESTONES):
        if total >= needed and claimed < needed:
            increment_question_limit(referrer_id, bonus)
            conn2 = db_connect()
            cur2 = conn2.cursor()
            cur2.execute("UPDATE users SET referral_milestone_claimed=? WHERE user_id=?", (needed, referrer_id))
            conn2.commit()
            conn2.close()
            try:
                await bot.send_message(
                    referrer_id,
                    f"🎁 **Referral Milestone!** You've referred {needed} people — "
                    f"+{bonus} bonus AI questions added to your account.",
                    parse_mode="Markdown",
                )
            except Exception:
                pass
...
async def refer_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    total = count_referrals(user_id)
    milestones_text = "\n".join(f"• {n} referrals → +{b} questions" for n, b in sorted(REFERRAL_MILESTONES))
    text = (
        f"{ui_header('Refer & Earn', '🎁')}\n"
        f"Share your link — you get **+10 AI questions** every time someone new joins through it:\n\n"
        f"`{link}`\n\n"
        f"👥 Total referred so far: **{total}**\n\n"
        f"🏆 **Milestone bonuses:**\n{milestones_text}"
    )
    await update.message.reply_text(text, parse_mode="Markdown")


async def refer_menu_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    await query.answer()
    user_id = update.effective_user.id
    bot_username = (await context.bot.get_me()).username
    ...
    await query.edit_message_text(text, parse_mode="Markdown", reply_markup=back_to_tools_keyboard())
...
async def start_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if context.args and context.args[0].startswith("ref_") and user_data['questions_used'] == 0:
        try:
            ...
            if referrer_id != user_id:
                ...
                await _maybe_grant_referral_milestone(context.bot, referrer_id)
        except ValueError:
            pass
    ...
    if not is_verified:
        ...
        await context.bot.send_message(update.effective_chat.id, verify_msg, parse_mode="Markdown", reply_markup=verification_keyboard())
        return
    ...
    await context.bot.send_message(
        ...
        build_welcome_text(user_name, user_data),
        parse_mode="Markdown",
...
async def help_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    help_text = (
        ...
        f"• `/importquiz` — 📥 Official Quiz Board se share kiya hua quiz customize karke save karo (apna tag, description, promotion link add karo)\n"
        ...
        f"• Question counts are 50/75/100. _In a **group**, `/topic`/`/pdf`/etc. also ask for a "
        f"timer + shuffle, then post live with a temporary leaderboard — same as Official Quiz. "
        f"In DM they just post all at once, no timer, like before._\n\n"
        ...
        f"• `/profile` or `/mystats` — your own stats, streak, and badges\n"
        f"• `/duel <topic>` — reply to someone in a group to challenge them to a 5-question 1v1\n\n"
        f"📁 **Quiz Sets & Library:**\n"
        f"• `/newset Name` — create a named folder for your quizzes\n"
        f"• `/sets` — browse your sets and what's filed in each\n"
        f"• `/browse [keyword]` — search your saved quizzes\n"
        f"• `/exportquiz <id>` — download a quiz as JSON\n"
        f"• `/exportresults <id>` — download a quiz's results as CSV\n\n"
        f"☀️ **Automation:**\n"
        f"• `/dailyquiz on <topic>` / `off` / `hour <0-23>` — auto-post a daily quiz to this group (admins only)\n\n"
        f"📤 **Channels:**\n"
        ...
        f"• `/limits` — see today's usage vs. daily cap for every key in your chain\n"
        ...
        f"💬 **Support:**\n"
        f"• `/feedback <message>` — send a message directly to the bot owner"
    )
...
async def create_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text(
        f"{ui_header('Tools & Features Menu', '🛠️')}\n\nSelect any tool below to proceed:",
...
# --- Live "Official Quiz" control by COMMAND (no buttons) ---------------------
# /pause /resume /stopquiz /undoquiz /fastquiz /slowquiz all act on whatever
# live_quizzes[chat_id] session is running in the chat they're sent in. Only the
# person who started it, or a chat admin, may control it — same rule as before.
LIVE_QUIZ_MIN_TIMER = 5
LIVE_QUIZ_MAX_TIMER = 120
LIVE_QUIZ_SPEED_STEP = 5


async def _get_controllable_live_session(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Returns the running session for this chat if the sender is allowed to
    control it, else replies with why not and returns None."""
    chat_id = update.effective_chat.id
    live = context.application.bot_data.get('live_quizzes', {})
    session = live.get(chat_id)
    if not session or session.get('finished'):
        await update.message.reply_text("There's no Official Quiz currently running in this chat.")
        return None
    user_id = update.effective_user.id
    is_admin = await is_chat_admin(update, context)
    if user_id != session['creator_id'] and not is_admin:
        await update.message.reply_text("Only the quiz creator or a chat admin can control this.")
        return None
    return session


async def pause_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    session = await _get_controllable_live_session(update, context)
    if session is None:
        # No live quiz here — keep the old generic behaviour so /pause still
        # makes sense mid-way through any other multi-step flow.
        await update.message.reply_text("⏸ Okay — I'll wait. Send /resume (or just carry on) whenever you're ready.")
        return
    session['paused'] = True
    await update.message.reply_text("⏸ Paused — send /resume whenever you're ready.")
...
async def stopquiz_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await _finish_live_quiz(context.bot, update.effective_chat.id, context.application)


async def undoquiz_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    session = await _get_controllable_live_session(update, context)
    if session is None:
        return
    session['undo_requested'] = True
    await update.message.reply_text("↩️ Undoing last question...")


async def fastquiz_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    session = await _get_controllable_live_session(update, context)
    if session is None:
        return
    current = session['timer'] or (LIVE_QUIZ_MIN_TIMER + LIVE_QUIZ_SPEED_STEP * 3)
    session['timer'] = max(LIVE_QUIZ_MIN_TIMER, current - LIVE_QUIZ_SPEED_STEP)
    await update.message.reply_text(
        f"🐇 Sped up — next question onward gets {session['timer']}s to answer."
    )


async def slowquiz_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    session = await _get_controllable_live_session(update, context)
    if session is None:
        return
    current = session['timer'] or (LIVE_QUIZ_MIN_TIMER + LIVE_QUIZ_SPEED_STEP * 3)
    session['timer'] = min(LIVE_QUIZ_MAX_TIMER, current + LIVE_QUIZ_SPEED_STEP)
    await update.message.reply_text(
        f"🐢 Slowed down — next question onward gets {session['timer']}s to answer."
    )
...
# (shared by topic / pdf / image / section)
# ==========================================
# Telegram's OWN hard limit (not this bot's, and not adjustable in code): a normal
# hosted bot — anything not running its own self-hosted "local Bot API server" — can
# NEVER download a file bigger than 20 MB via getFile, full stop. Sending files out
# allows more (up to 50 MB via upload), but that's a different limit for a different
# direction and doesn't help here. Checked from msg.document.file_size (already in
# the update, no extra API call) so an oversized PDF gets an instant, clear message
# instead of a silent/confusing failure from Telegram partway through a download.
MAX_PDF_SIZE_MB = 20
...
async def receive_content(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if source_type == 'pdf':
        ...
        if msg.document.file_size and msg.document.file_size > MAX_PDF_SIZE_BYTES:
            await msg.reply_text(
                f"❌ Ye PDF **{size_mb:.1f} MB** ki hai — Telegram bots (bina self-hosted "
                f"local Bot API server ke) {MAX_PDF_SIZE_MB} MB se badi file download hi "
                f"nahi kar sakte. Ye Telegram ki apni hard limit hai (getFile API), is bot "
                f"ka koi bug nahi — koi bhi normal-hosted Telegram bot isse bypass nahi kar "
                f"sakta.\n\nChhoti PDF bhejo, ya usme se relevant pages nikaal ke bhejo.",
                parse_mode="Markdown",
            )
            return ASK_CONTENT
        status_msg = await msg.reply_text(f"📄 {size_mb:.1f} MB PDF mila — padh raha hoon...")
        try:
            file = await msg.document.get_file()
            data = await file.download_as_bytearray()
        except Exception as e:
            await status_msg.edit_text(f"❌ PDF download nahi ho paayi: {e}")
            return ASK_CONTENT
        ...
        try:
            ...
            pass  # cosmetic only — a stale/undeletable status message is harmless
        return await ask_difficulty(update, context)
    ...
    if source_type == 'section':
        ...
        context.user_data['gen_owner_id'] = owner_id
        keyboard = InlineKeyboardMarkup([[
            InlineKeyboardButton(str(n), callback_data=_tag_cb(f"sec_{n}", owner_id)) for n in (2, 3, 4, 5)
        ]])
...
def _tag_cb(data: str, owner_id: int) -> str:
    """Stamps the id of the person the button was shown to onto the callback_data
    (separated by '|', which never appears in our own prefixes/values). Any group
    member can SEE an inline keyboard, but only the person it was meant for should
    be able to act on it — without this, a quiz-setup button (difficulty/language/
    timer/count) posted in a group can be tapped by anyone else in that group and
    silently redirect the host's own in-progress quiz setup."""
    return f"{data}|{owner_id}"


def _untag_cb(data: str):
    """Reverses _tag_cb. Returns (payload, owner_id) — owner_id is None for any
    legacy/foreign callback_data that never went through _tag_cb, so callers can
    fail safe (treat untagged data as belonging to no one)."""
    if "|" in data:
        payload, raw_id = data.rsplit("|", 1)
        try:
            return payload, int(raw_id)
        except ValueError:
            return payload, None
    return data, None


async def _reject_foreign_tap(query) -> None:
    await query.answer("⛔ Ye button aapke liye nahi hai — apna khud ka /topic, /pdf ya /image chalayein.", show_alert=True)
...
async def ask_difficulty(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    keyboard = InlineKeyboardMarkup([[
        InlineKeyboardButton("🟢 EASY", callback_data=_tag_cb("diff_easy", owner_id)),
        InlineKeyboardButton("🟡 MEDIUM", callback_data=_tag_cb("diff_medium", owner_id)),
        InlineKeyboardButton("🔴 HARD", callback_data=_tag_cb("diff_hard", owner_id)),
    ]])
    await context.bot.send_message(chat_id, f"{ui_header('Select Difficulty', '🎚')}", reply_markup=keyboard)
    return ASK_DIFFICULTY


async def receive_difficulty(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    payload, owner_id = _untag_cb(query.data)
    if owner_id is not None and query.from_user.id != owner_id:
        await _reject_foreign_tap(query)
        return ASK_DIFFICULTY
    await query.answer()
    context.user_data['gen_difficulty'] = payload.replace('diff_', '')
    await query.edit_message_reply_markup(reply_markup=None)
    return await ask_language(update, context)


async def ask_language(update: Update, context: ContextTypes.DEFAULT_TYPE):
    owner_id = context.user_data.get('gen_owner_id', update.effective_user.id)
    user = get_user(owner_id)
    chat_id = update.effective_chat.id
    if user['default_language']:
        context.user_data['gen_language'] = user['default_language']
        return await after_language(update, context)
    keyboard = InlineKeyboardMarkup([
        [InlineKeyboardButton("🇬🇧 ENGLISH", callback_data=_tag_cb("lang_English", owner_id)),
         InlineKeyboardButton("🇮🇳 HINDI", callback_data=_tag_cb("lang_Hindi", owner_id))],
    ])
    await context.bot.send_message(chat_id, f"{ui_header('Select Language', '🌐')}", reply_markup=keyboard)
    return ASK_LANGUAGE
...
async def receive_language_button(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await query.answer()
    await query.edit_message_reply_markup(reply_markup=None)
    context.user_data['gen_language'] = payload.replace('lang_', '')
    return await after_language(update, context)


async def receive_language_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    # Language is strictly English or Hindi via the buttons above — no free-text
    # option anymore, so any typed message here is just a nudge back to them.
    await update.message.reply_text("Please tap English or Hindi above 👆")
    return ASK_LANGUAGE


async def after_language(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Groups get 2 extra steps (timer, then shuffle) before the count prompt, so the
    quiz can be posted live with a per-question timer and a temporary leaderboard —
    DMs skip straight to the count prompt exactly like before (no timer/leaderboard
    makes sense for a solo chat, and it keeps the DM flow fast)."""
    if update.effective_chat.type in ("group", "supergroup") and not context.user_data.get('gen_count'):
        return await ask_ai_timer(update, context)
    return await ask_count(update, context)


async def ask_ai_timer(update: Update, context: ContextTypes.DEFAULT_TYPE):
    owner_id = context.user_data.get('gen_owner_id', update.effective_user.id)
    keyboard = InlineKeyboardMarkup([
        [InlineKeyboardButton("10s", callback_data=_tag_cb("aitimer_10", owner_id)),
         InlineKeyboardButton("15s", callback_data=_tag_cb("aitimer_15", owner_id)),
         InlineKeyboardButton("20s", callback_data=_tag_cb("aitimer_20", owner_id))],
        [InlineKeyboardButton("30s", callback_data=_tag_cb("aitimer_30", owner_id)),
         InlineKeyboardButton("60s", callback_data=_tag_cb("aitimer_60", owner_id)),
         InlineKeyboardButton("⏱ No Timer", callback_data=_tag_cb("aitimer_0", owner_id))],
    ])
    await context.bot.send_message(
        update.effective_chat.id,
        f"{ui_header('Set Timer', '⏱')}\n\nGroup me polls live post honge — har question ke liye timer chuno:",
        reply_markup=keyboard,
    )
    return ASK_AI_TIMER


async def receive_ai_timer(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    payload, owner_id = _untag_cb(query.data)
    if owner_id is not None and query.from_user.id != owner_id:
        await _reject_foreign_tap(query)
    ...
    await query.answer()
    await query.edit_message_reply_markup(reply_markup=None)
    context.user_data['gen_timer'] = int(payload.replace('aitimer_', ''))
    keyboard = InlineKeyboardMarkup([[
        InlineKeyboardButton("🔀 SHUFFLE", callback_data=_tag_cb("aishuffle_yes", owner_id)),
        InlineKeyboardButton("↔️ KEEP ORDER", callback_data=_tag_cb("aishuffle_no", owner_id)),
    ]])
    await context.bot.send_message(
        update.effective_chat.id,
        f"{ui_header('Shuffle Options', '🔀')}\n\nHar poll me options ka order shuffle karein?",
        reply_markup=keyboard,
    )
    return ASK_AI_SHUFFLE


async def receive_ai_shuffle(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    payload, owner_id = _untag_cb(query.data)
    if owner_id is not None and query.from_user.id != owner_id:
        await _reject_foreign_tap(query)
    ...
    await query.answer()
    await query.edit_message_reply_markup(reply_markup=None)
    context.user_data['gen_shuffle'] = (payload == "aishuffle_yes")
    return await ask_count(update, context)


def shuffle_question_options(q: dict) -> dict:
    """Returns a copy of the question with its options (and correct_index) shuffled —
    the underlying MCQ content/correctness is untouched, only display order changes."""
...
async def ask_count(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if context.user_data.get('gen_count'):
        ...
        await context.bot.send_message(chat_id, ui_status("Generating your quiz..."))
        return await run_generation(update, context)
    keyboard = InlineKeyboardMarkup([[
        InlineKeyboardButton("🎯 25", callback_data=_tag_cb("cnt_25", owner_id)),
        InlineKeyboardButton("🎯 50", callback_data=_tag_cb("cnt_50", owner_id)),
        InlineKeyboardButton("🎯 75", callback_data=_tag_cb("cnt_75", owner_id)),
        InlineKeyboardButton("🎯 100", callback_data=_tag_cb("cnt_100", owner_id)),
    ]])
    await context.bot.send_message(chat_id, f"{ui_header('Question Count', '🎯')}", reply_markup=keyboard)
    return ASK_COUNT
...
async def receive_count(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await query.answer()
    await query.edit_message_reply_markup(reply_markup=None)
    context.user_data['gen_count'] = int(payload.replace('cnt_', ''))
    await context.bot.send_message(update.effective_chat.id, ui_status("Generating your quiz..."))
    return await run_generation(update, context)
...
async def run_generation(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    is_group_ai_flow = context.user_data.get('gen_timer') is not None
    if is_group_ai_flow:
        # Group flow: paced, timed posting + a live/temporary leaderboard, via the same
        # engine /startquiz uses — auto-saved first so it's replayable later too.
        if context.user_data.get('gen_shuffle'):
            questions = [shuffle_question_options(q) for q in questions]
        timer = context.user_data.get('gen_timer', 0)
        ...
        increment_questions_used(user_id, len(questions))
        record_daily_ai_usage(user_id, len(questions))

        timer_text = f"{timer}s / question" if timer else "no limit"
        await context.bot.send_message(
            chat_id,
            f"✅ {len(questions)} question(s) generated — Quiz ID `{quiz_id}` (saved, replay anytime with "
            f"`/startquiz {quiz_id}`). Timer: {timer_text}. Starting now...",
            parse_mode="Markdown",
        )
        await initiate_live_quiz(context.bot, context.application, chat_id, update.effective_chat.type,
                                  quiz_id, user_id, update.effective_user.first_name or "Host")
        context.user_data.clear()
        return ConversationHandler.END
...
async def receive_voice(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    data = await file.download_as_bytearray()
    await msg.reply_text(ui_status("Sun raha hoon..."))
...
async def import_entry(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    text = (
        f"{ui_header('Import from Channel or Group', '📥')}\n\n"
...
# ==========================================
# 📥 QUIZ IMPORT & CUSTOMIZE (from official quiz board)
# ==========================================
async def importquiz_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Start quiz import flow — user forwards a quiz from official quiz board."""
    msg = update.effective_message
    user_id = update.effective_user.id
    user = get_user(user_id)
    
    msg_text = (
        f"{ui_header('Import & Customize Quiz', '📥')}\n\n"
        "Telegram ke official Quiz Board se jo quiz share karte ho (jaise @quizzesbot se), "
        "usi ko **forward** karo is chat me.\n\n"
        "Main us quiz ko analyze kar dunga aur tum apna:\n"
        "• **Tag** (har question ke saath)\n"
        "• **Description** (quiz ka overview)\n"
        "• **Promotion Link** (explanation me add hoga)\n\n"
        "...apna kuch bhi daalna ho to bata dena, main customized version save kar dunga."
    )
    
    sent = await msg.reply_text(msg_text, parse_mode="Markdown", reply_markup=cancel_keyboard())
    context.user_data['importquiz_msg_id'] = sent.message_id
    return IMPORT_QUIZ_POLL


async def receive_importquiz_poll(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Receive the forwarded quiz poll."""
    msg = update.effective_message
    poll = msg.poll
    
    if not poll:
        await msg.reply_text("❌ Ye ek poll nahi hai. Telegram Quiz Board se koi quiz forward karo.")
        return IMPORT_QUIZ_POLL
    
    if poll.type != Poll.QUIZ:
        await msg.reply_text("❌ Ye ek quiz poll nahi hai — general poll hai. Official quiz board se forward karo.")
        return IMPORT_QUIZ_POLL
    
    # Store the poll data
    context.user_data['importquiz_poll'] = {
    ...
    # Ask for tag
    await msg.reply_text(
        "✅ Quiz capture ho gaya!\n\n"
        "Ab **apna tag** kya hoga? (jaise 'Physics Lab' ya '@yourname')\n"
        "...ya *skip* kar do agar koi tag nahi chaho.",
        parse_mode="Markdown",
        reply_markup=cancel_keyboard(),
    )
    return IMPORT_QUIZ_TAG


async def receive_importquiz_tag(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Receive custom tag."""
    msg = update.effective_message
    ...
    context.user_data['importquiz_tag'] = tag[:100]
    
    await msg.reply_text(
        "👍 Tag set!\n\n"
        "Ab **apna description** kya hoga? (quiz ka overview — 1-2 lines)\n"
        "...ya *skip* kar do.",
        parse_mode="Markdown",
        reply_markup=cancel_keyboard(),
    )
    return IMPORT_QUIZ_DESC


async def receive_importquiz_desc(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Receive custom description."""
    msg = update.effective_message
    ...
    context.user_data['importquiz_desc'] = desc[:200]
    
    await msg.reply_text(
        "✅ Description set!\n\n"
        "Ab **apna promotion link** kya hoga? (jaise Telegram channel ya website)\n"
        "...ya *skip* kar do.",
        parse_mode="Markdown",
        reply_markup=cancel_keyboard(),
    )
    return IMPORT_QUIZ_PROMO


async def receive_importquiz_promo(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Receive promo link and save the customized quiz."""
    msg = update.effective_message
    promo = (msg.text or '').strip()
    
    if promo.lower() == 'skip':
        promo = ''
    
    context.user_data['importquiz_promo'] = promo[:150]
    
    # Build the customized question
    poll_data = context.user_data.get('importquiz_poll')
    tag = context.user_data.get('importquiz_tag', '')
    desc = context.user_data.get('importquiz_desc', '')
    promo = context.user_data.get('importquiz_promo', '')
    
    # Create a single question from the poll (or we can create multiple if it's a batch)
    question_text = poll_data['question']
    if tag:
        question_text = f"{question_text}\n\n🏷 {tag}"
    ...
    questions = [{
        ...
        'correct_index': min(poll_data['correct_index'], 3),
        'explanation': explanation[:195],
    }]
    
    # Save as a new quiz
    try:
        title = f"Imported: {poll_data['question'][:40]}"
        quiz_id = save_quiz(
            update.effective_user.id,
            title,
            questions,
            description=desc,
            creator_name=tag or "Imported",
        )
        
        await msg.reply_text(
            f"✅ **Quiz Save ho gaya!**\n\n"
            f"📌 Quiz ID: `{quiz_id}`\n"
            f"📝 Question: {poll_data['question'][:60]}...\n"
            f"🏷 Tag: {tag or '(none)'}\n"
            f"📋 Desc: {desc or '(none)'}\n"
            f"🔗 Promo: {promo or '(none)'}\n\n"
            f"`/startquiz {quiz_id}` likhkar start kar sakte ho.",
            parse_mode="Markdown",
        )
    except Exception as e:
        logger.exception(f"Quiz import save failed: {e}")
        await msg.reply_text(f"❌ Error saving quiz: {e}")
    
    # Cleanup
    context.user_data.pop('importquiz_poll', None)
    context.user_data.pop('importquiz_tag', None)
    context.user_data.pop('importquiz_desc', None)
    context.user_data.pop('importquiz_promo', None)
    context.user_data.pop('importquiz_msg_id', None)
...
async def oq_finalize(update: Update, context: ContextTypes.DEFAULT_TYPE, query=None):
    ...
    try:
        ...
        # Only oq_questions/oq_title/oq_timer are needed to retry — don't clear
        # them here, or the "tap Done again" above would be a lie and the retry
        # would always fail with "no questions parsed".
        return OQ_COLLECT if context.user_data.get('oq_collecting') is not None else OQ_BULK

    timer_text = f"{timer}s / question" if timer else "No limit"
    summary = (
        f"{ui_header('Official Quiz Ready', '✅')}\n\n"
        f"📝 **Title:** {title}\n"
        f"⏱ **Timer:** {timer_text}\n"
        f"🆔 **Quiz ID:** `{quiz_id}`\n"
...
async def start_live_quiz(bot, application, chat_id, quiz_id, started_by_id, started_by_name, is_dm=False, battle_royale=False):
    quiz = get_quiz_meta(quiz_id)
    if not quiz:
    ...
    session = {
        ...
        "author_option_style": creator['option_style'], "author_poll_style": creator['poll_style'],
        "poll_map": {}, "poll_sent_at": {}, "scores": {}, "last_poll_msg_id": None,
        "current_poll_id": None, "answers_this_question": 0, "consecutive_silent": 0,
        "is_dm": is_dm, "battle_royale": battle_royale,
    }
    ...
    await bot.send_message(
        chat_id,
        f"{ui_header('Official Quiz — Live', '🚦')}\n📝 {quiz['title']}\n"
        + (f"💬 {quiz['description']}\n" if quiz.get('description') else "") +
        f"👤 By: {author_mention} (`{session['author_id']}`)\n"
        f"⏱ {timer_text} · ❓ {len(quiz['questions'])} questions\n\n"
        f"Only {started_by_name} or a chat admin can control this — with:\n"
        f"⏸ /pause · ▶️ /resume · ⏹ /stopquiz · ↩️ /undoquiz\n"
        f"🐇 /fastquiz (speed up) · 🐢 /slowquiz (slow down)",
        parse_mode="Markdown",
    )
...
# The host is auto-readied on creation, and a "▶️ Start Now" button lets the host/an
# admin skip the wait entirely — without these two, a quiet group could sit at the
# lobby screen until the 10-minute timeout with no way to just start anyway.
# ==========================================
LOBBY_MIN_READY = 2
...
def _lobby_view(chat_id, lobby):
    ...
    keyboard = InlineKeyboardMarkup([
        ...
        [InlineKeyboardButton("▶️ Start Now (host/admin)", callback_data=f"oqstartnow_{chat_id}")],
...
async def initiate_live_quiz(bot, application, chat_id, chat_type, quiz_id, requested_by_id, requested_by_name):
    ...
    if chat_type not in ("group", "supergroup"):
        await start_live_quiz(bot, application, chat_id, quiz_id, requested_by_id, requested_by_name, is_dm=True)
        return
    ...
    lobbies[chat_id] = {
        ...
        # The person who typed /startquiz obviously wants to play — count them as
        # ready immediately instead of silently requiring them to tap the button a
        # SECOND time too (this alone made small/quiet groups look "stuck" at 0/2).
        "ready_users": {requested_by_id}, "message_id": None,
    }
...
async def lobby_startnow_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """The host or a chat admin can skip the LOBBY_MIN_READY wait entirely and start
    with however many people are ready right now (at least the host themself, since
    they're auto-readied above) — the fix for a lobby that would otherwise sit there
    until the 10-minute timeout in a small/quiet group where a 2nd tap never comes."""
    query = update.callback_query
    chat_id = int(query.data.replace("oqstartnow_", ""))
    lobbies = context.application.bot_data.get('quiz_lobbies', {})
    lobby = lobbies.get(chat_id)
    if not lobby:
        await query.answer("This waiting room has closed.", show_alert=True)
    ...
    is_admin = await is_chat_admin(update, context)
    if query.from_user.id != lobby['requested_by_id'] and not is_admin:
        await query.answer("Only whoever started this, or a chat admin, can start early.", show_alert=True)
        return
    lobbies.pop(chat_id, None)
    try:
        await query.edit_message_text(f"▶️ Starting now with {len(lobby['ready_users'])} player(s)!")
    except Exception as e:
        logger.warning(f"lobby_startnow_callback edit failed (non-fatal): {e}")
    await start_live_quiz(context.bot, context.application, chat_id, lobby['quiz_id'],
                           lobby['requested_by_id'], lobby['requested_by_name'])
...
# How long to actually wait on a question when the host picked "⏱ No Timer". Telegram
# poll open_period tops out at 600s anyway, so this doubles as a generous, genuinely
# "no rush" window rather than looping forever waiting for 100% of a group to answer.
NO_TIMER_SECONDS_PER_QUESTION = 120
...
async def _run_live_quiz_loop(bot, application, chat_id):
    ...
    try:
        while session['index'] < len(session['questions']) and not session['stopped']:
            ...
            question_text, explanation = apply_poll_style(question_text, explanation, session.get('author_poll_style', 'native'))
            # A transient network hiccup or a momentary Telegram rate-limit on send_poll
            # used to just skip that question outright — and since a skipped question
            # sends nothing for anyone to answer, 2 in a row would trip the silent-
            # question auto-pause above, making the WHOLE quiz look like it "randomly
            # stopped" from a single blip. Retry a couple of times first; only skip the
            # question if it's still failing after that.
            msg = None
            for attempt in range(3):
                try:
                    ...
                    if attempt < 2:
                        logger.warning(f"Live quiz poll send failed, retrying ({attempt + 1}/3): {e}")
                        await asyncio.sleep(2 * (attempt + 1))
                    else:
                        logger.warning(f"Live quiz poll send failed after 3 tries (skipping this question): {e}")
            if msg:
                ...
                # Live Doubt Button — DM only. A group quiz sending one extra message
                # per question would double the chat's message volume and risk
                # Telegram rate limits on a big quiz; in a DM it's one person, so the
                # extra tap-for-detail is a clean win with no spam/rate-limit downside.
                if session.get('is_dm'):
                    _remember_doubt_poll(application, msg.poll.id, q)
                    try:
                        await bot.send_message(
                            chat_id, "🤔 Confused on this one?",
                            reply_markup=InlineKeyboardMarkup(
                                [[InlineKeyboardButton("❓ Doubt (detailed explain)", callback_data=f"doubtbtn_{msg.poll.id}")]]
                            ),
                        )
                    except Exception:
                        pass
            ...
            # timer == 0 means the host explicitly picked "⏱ No Timer" (unlimited time
            # per question) — that's a different thing from timer being unset, so it
            # must NOT fall back to the 15s default below (it used to, silently capping
            # every "No Timer" quiz at ~17s/question — the opposite of what was picked,
            # and a very plausible source of "scores look wrong": people were still
            # mid-read when the next question fired and got marked as no-answer).
            if session['timer'] == 0:
                wait_seconds = NO_TIMER_SECONDS_PER_QUESTION
            else:
            ...
            while elapsed < wait_seconds:
                ...
                if session.get('undo_requested'):
                    ...
                    undone = True
                    break
                # In a DM it's a single player, so there's no point waiting out the
                # rest of the per-question timer once they've already answered — the
                # next question can fire immediately. (Groups keep the full timer
                # since other players may still be reading/answering.)
                if session.get('is_dm') and session['answers_this_question'] > 0:
                    break
...
async def _finish_live_quiz(bot, chat_id, application):
    ...
    if ranking:
        ...
        board_text = (
            f"{ui_header('Leaderboard', '🏆')}\n📝 {session['title']}\n\n" + "\n".join(lines) +
            "\n\n👤 User ID is shown next to each name so the host can reach out directly (prizes, verification, etc.)."
        )
    else:
        board_text = f"{ui_header('Leaderboard', '🏆')}\n📝 {session['title']}\n\nNo answers were recorded."

    extra_lines = []

    # 👻 Ghost Mode — DM solo quiz only: compare this run against your own best
    # ever run on this exact quiz, stored as JSON on the user row. Wrapped so a
    # bad/missing JSON value can never break the leaderboard send.
    if session.get('is_dm') and ranking:
        try:
            solo = ranking[0]
            user = get_user(solo['user_id'])
            best_map = json.loads(user['best_solo_score'] or '{}')
            key = str(session['quiz_id'])
            prev_best = best_map.get(key, {}).get('score')
            if prev_best is None:
                extra_lines.append(f"👻 **Ghost Mode**: first run on this quiz — {solo['score']} pts sets your Ghost!")
                best_map[key] = {"score": solo['score']}
            elif solo['score'] > prev_best:
                extra_lines.append(f"👻 **Ghost Mode**: New Best! {solo['score']} pts (previous Ghost: {prev_best})")
                best_map[key] = {"score": solo['score']}
            else:
                extra_lines.append(f"👻 **Ghost Mode**: {solo['score']} pts vs your Ghost's {prev_best} pts — beat it next time!")
            conn = db_connect()
            try:
                cur = conn.cursor()
                cur.execute("UPDATE users SET best_solo_score=? WHERE user_id=?",
                            (json.dumps(best_map), solo['user_id']))
                conn.commit()
            finally:
                conn.close()
        except Exception as e:
            logger.warning(f"Ghost Mode comparison failed (non-fatal): {e}")

    # 🎁 Mystery Box — one random participant (not necessarily the winner) gets a
    # small surprise reward. Appended as text only, no extra message/API call.
    if ranking:
        try:
            lucky = random.choice(ranking)
            roll = random.random()
            if roll < 0.08:
                # rare: a free streak-freeze token
                conn = db_connect()
                try:
                    cur = conn.cursor()
                    cur.execute("UPDATE users SET streak_freeze_count = streak_freeze_count + 1 WHERE user_id=?",
                                (lucky['user_id'],))
                    conn.commit()
                finally:
                    conn.close()
                extra_lines.append(f"🎁 **Mystery Box**: {lucky['name']} found a ❄️ Streak Freeze token!")
            else:
                reward = random.randint(5, 25)
                adjust_coins(lucky['user_id'], reward)
                extra_lines.append(f"🎁 **Mystery Box**: {lucky['name']} found **{reward} coins**!")
        except Exception as e:
            logger.warning(f"Mystery Box failed (non-fatal): {e}")

    # ⚔️ Battle Royale flavor — labels a flawless run as a Survivor vs anyone who
    # missed at least one question. (Telegram's poll API has no way to actually
    # stop someone from answering once "eliminated" — this is a scoring/labeling
    # twist on the same leaderboard, not a real-time block.)
    if session.get('battle_royale') and ranking:
        survivors = [s['name'] for s in ranking if s['wrong'] == 0 and s['answered'] > 0]
        if survivors:
            extra_lines.append("⚔️ **Battle Royale Survivors** (flawless run): " + ", ".join(survivors[:15]))
        else:
            extra_lines.append("⚔️ **Battle Royale**: nobody survived flawless this round!")
    ...
    # 📜 Auto-certificate for the top scorer only (avoids spamming a big group with
    # an image per player). Wrapped so a certificate failure can never affect the
    # rest of quiz completion — this is a pure bonus, never load-bearing.
    if ranking:
        top = ranking[0]
        try:
            accuracy = round((top['correct'] / top['answered']) * 100, 1) if top['answered'] else 0.0
            buf = await asyncio.to_thread(
                generate_certificate_image, top['name'], session['title'], top['score'],
                top['correct'], top['wrong'], accuracy, rank_label="🥇 1st Place",
            )
            await bot.send_photo(chat_id, buf, caption=f"📜 Congrats {top['name']}! Here's your certificate.")
        except Exception as e:
            logger.warning(f"Auto-certificate for top scorer failed (non-fatal): {e}")

    # 🏆 Squad Tournament — no-op unless this chat is genuinely one side of an
    # active tournament on this exact quiz; wrapped so it can never affect normal
    # quiz completion either.
    try:
        combined_score = sum(s['score'] for s in ranking)
        await record_tournament_score(bot, chat_id, session['quiz_id'], combined_score)
    except Exception as e:
        logger.warning(f"record_tournament_score failed (non-fatal): {e}")
...
# ==========================================
def _certificate_font(size: int, bold: bool = False):
    # Render's base image may or may not have these fonts installed — try a few
    # common paths and fall back to PIL's built-in bitmap font (uglier, but this
    # can never crash the certificate feature just because a TTF isn't present).
    candidates = [
...
def generate_certificate_image(name: str, quiz_title: str, score: int, correct: int,
                                 wrong: int, accuracy: float, rank_label: str = "") -> io.BytesIO:
    W, H = 1000, 650
    img = Image.new("RGB", (W, H), color=(15, 17, 26))
    draw = ImageDraw.Draw(img)

    draw.rectangle([15, 15, W - 15, H - 15], outline=(212, 175, 55), width=6)
    draw.rectangle([30, 30, W - 30, H - 30], outline=(212, 175, 55), width=2)

    title_font = _certificate_font(46, bold=True)
    name_font = _certificate_font(38, bold=True)
    label_font = _certificate_font(22)
    stat_font = _certificate_font(26, bold=True)

    def center_text(y, text, font, fill=(230, 230, 230)):
        bbox = draw.textbbox((0, 0), text, font=font)
        w = bbox[2] - bbox[0]
        draw.text(((W - w) / 2, y), text, font=font, fill=fill)

    center_text(60, "🏆 CERTIFICATE OF ACHIEVEMENT", title_font, fill=(212, 175, 55))
    center_text(150, "This is proudly presented to", label_font)
    center_text(185, name[:40], name_font, fill=(255, 255, 255))
    center_text(250, "for completing", label_font)
    center_text(280, quiz_title[:60] or "a quiz", label_font, fill=(180, 200, 255))

    if rank_label:
        center_text(330, rank_label, stat_font, fill=(212, 175, 55))

    stats_y = 400
    stats = [f"Score: {score}", f"✅ {correct}   ❌ {wrong}", f"Accuracy: {accuracy}%"]
    for i, s in enumerate(stats):
        center_text(stats_y + i * 40, s, stat_font, fill=(230, 230, 230))

    center_text(H - 60, datetime.utcnow().strftime("%d %b %Y"), label_font, fill=(150, 150, 150))
...
async def certificate_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """/certificate — generates a shareable achievement card from the caller's
    all-time stats. Works in any chat, any time — not tied to a specific quiz."""
    stats = get_user_profile_stats(update.effective_user.id)
    if stats['quizzes_played'] == 0:
        await update.message.reply_text("📜 Play at least one quiz first, then come back for your certificate!")
        return
    try:
        buf = await asyncio.to_thread(
            generate_certificate_image, update.effective_user.first_name, "CyberQuiz Bot",
            stats['correct'] * 10, stats['correct'], stats['wrong'], stats['accuracy'],
            rank_label=f"🔥 {stats['streak']}-day streak" if stats['streak'] else "",
        )
        await update.message.reply_photo(buf, caption="📜 Your achievement certificate — share it anywhere!")
    except Exception as e:
        logger.warning(f"/certificate generation failed: {e}")
        await update.message.reply_text("⚠️ Couldn't generate the certificate right now — try again in a bit.")


# ==========================================
# 🏆 SQUAD TOURNAMENT (group vs group, on the same quiz)
# ==========================================
async def tournament_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """/tournament <quiz_id> — run in Group A to open a challenge on a specific
    saved quiz. Gives back a tournament number; the OTHER group joins with
    /jointournament <that number>. Each group then plays the quiz normally
    (Official Quiz start / /startquiz) — whichever group's combined score is
    higher when both are done wins. (Telegram strips inline buttons from
    forwarded messages, so joining is a typed command rather than a button —
    this keeps it reliable across two separate chats.)"""
    if update.effective_chat.type not in ("group", "supergroup"):
        await update.message.reply_text("🏆 /tournament only works inside a group.")
        return
    if not context.args or not context.args[0].isdigit():
        await update.message.reply_text("Usage: `/tournament <quiz_id>` — e.g. `/tournament 42`", parse_mode="Markdown")
        return
    quiz_id = int(context.args[0])
    quiz = get_quiz_by_id(quiz_id)
    if not quiz:
        await update.message.reply_text("⚠️ That quiz ID doesn't exist.")
        return
    conn = db_connect()
    try:
        cur = conn.cursor()
        cur.execute(
            "INSERT INTO tournaments (quiz_id, group_a_id, group_a_name, created_by, status) "
            "VALUES (?, ?, ?, ?, 'waiting')",
            (quiz_id, update.effective_chat.id, (update.effective_chat.title or "")[:120], update.effective_user.id),
        )
        tournament_id = last_insert_id(cur, conn, "tournaments")
        conn.commit()
    finally:
        conn.close()
    await update.message.reply_text(
        f"{ui_header('Tournament Created', '🏆')}\n"
        f"Challenge on quiz **{quiz['title']}** (#{quiz_id}) is open.\n\n"
        f"In the **other group**, an admin should run:\n`/jointournament {tournament_id}`\n\n"
        f"Once they join, both groups play the same quiz (`/startquiz {quiz_id}` or the Official "
        f"Quiz start button) — highest combined group score wins!",
        parse_mode="Markdown",
    )


async def join_tournament_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if update.effective_chat.type not in ("group", "supergroup"):
        await update.message.reply_text("🏆 /jointournament only works inside a group.")
        return
    if not context.args or not context.args[0].isdigit():
        await update.message.reply_text("Usage: `/jointournament <tournament_id>`", parse_mode="Markdown")
        return
    tid = int(context.args[0])
    conn = db_connect()
    try:
        cur = conn.cursor()
        cur.execute("SELECT quiz_id, group_a_id, group_a_name, status FROM tournaments WHERE id=?", (tid,))
        row = cur.fetchone()
        if not row:
            await update.message.reply_text("⚠️ No tournament with that number.")
            return
        quiz_id, group_a_id, group_a_name, status = row
        if status != 'waiting':
            await update.message.reply_text("⚠️ That tournament isn't open to join anymore.")
            return
        if group_a_id == update.effective_chat.id:
            await update.message.reply_text("⚠️ That's the same group that created it — invite a *different* group.", parse_mode="Markdown")
            return
        cur.execute(
            "UPDATE tournaments SET group_b_id=?, group_b_name=?, status='ready' WHERE id=?",
            (update.effective_chat.id, (update.effective_chat.title or "")[:120], tid),
        )
        conn.commit()
    finally:
        conn.close()
    quiz = get_quiz_by_id(quiz_id)
    announce = (
        f"{ui_header('Tournament Ready!', '⚔️')}\n**{group_a_name}** vs **{update.effective_chat.title}**\n"
        f"Quiz: {quiz['title'] if quiz else quiz_id}\n\n"
        f"Both groups: play it now with `/startquiz {quiz_id}` — combined group score decides the winner!"
    )
    try:
        await context.bot.send_message(group_a_id, announce, parse_mode="Markdown")
    except Exception:
        pass
    await update.message.reply_text(announce, parse_mode="Markdown")


async def record_tournament_score(bot, chat_id: int, quiz_id: int, total_score: int):
    """Called after a live quiz finishes (see _finish_live_quiz). If this chat is
    one side of a 'ready' tournament on this exact quiz, records this group's
    combined score; once BOTH sides are in, announces the winner in both groups
    and marks the tournament done. Silently does nothing if there's no matching
    tournament — most quiz completions have nothing to do with a tournament."""
    conn = db_connect()
    try:
        cur = conn.cursor()
        cur.execute(
            "SELECT id, group_a_id, group_a_name, group_a_score, group_b_id, group_b_name, group_b_score "
            "FROM tournaments WHERE quiz_id=? AND status='ready' AND (group_a_id=? OR group_b_id=?)",
            (quiz_id, chat_id, chat_id),
        )
        row = cur.fetchone()
        if not row:
            return
        tid, a_id, a_name, a_score, b_id, b_name, b_score = row
        if chat_id == a_id:
            a_score = total_score
            cur.execute("UPDATE tournaments SET group_a_score=? WHERE id=?", (total_score, tid))
        else:
            b_score = total_score
            cur.execute("UPDATE tournaments SET group_b_score=? WHERE id=?", (total_score, tid))
        conn.commit()
        if a_score is not None and b_score is not None:
            cur.execute("UPDATE tournaments SET status='done' WHERE id=?", (tid,))
            conn.commit()
            if a_score > b_score:
                winner_line = f"🏆 **{a_name}** wins! ({a_score} vs {b_score})"
            elif b_score > a_score:
                winner_line = f"🏆 **{b_name}** wins! ({b_score} vs {a_score})"
            else:
                winner_line = f"🤝 It's a tie! ({a_score} vs {b_score})"
            result_text = f"{ui_header('Tournament Result', '⚔️')}\n{a_name}: {a_score}\n{b_name}: {b_score}\n\n{winner_line}"
            for target in (a_id, b_id):
                try:
                    await bot.send_message(target, result_text, parse_mode="Markdown")
                except Exception:
                    pass
    finally:
        conn.close()
...
async def handle_poll_answer(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for chat_id, session in live.items():
        if ans.poll_id in session['poll_map']:
            ...
            timer_cap_ms = (NO_TIMER_SECONDS_PER_QUESTION if session['timer'] == 0
                             else (session['timer'] or 15) + 2) * 1000
            ...
            if correct:
                ...
                entry['wrong'] += 1
                log_wrong_answer(ans.user.id, question, session.get('title', ''))
            ...
            return  # handled as part of a live-quiz session — done

    # Not part of any in-memory live-quiz session — check whether this is an AI
    # Mode broadcast poll instead (DB-backed, see ai_mode_polls in init_db, so
    # scoring still works even if the bot restarted between sending it and now).
    conn = db_connect()
    cur = conn.cursor()
    cur.execute("SELECT chat_id, correct_option_id FROM ai_mode_polls WHERE poll_id=?", (ans.poll_id,))
    row = cur.fetchone()
    conn.close()
    if not row:
        return
    ai_chat_id, correct_option_id = row
    correct = correct_option_id in ans.option_ids
    record_ai_mode_answer(ai_chat_id, ans.user.id, ans.user.first_name or "Player", correct)


# ==========================================
# 🎯 WEAK-TOPIC AUTO-REVISION
# ==========================================
MAX_WRONG_ANSWERS_PER_USER = 200  # keeps this table from growing forever for a very active user


def log_wrong_answer(user_id: int, question: dict, topic: str = ''):
    conn = db_connect()
    try:
        cur = conn.cursor()
        cur.execute(
            "INSERT INTO wrong_answers (user_id, question, options, correct_index, explanation, topic) "
            "VALUES (?, ?, ?, ?, ?, ?)",
            (user_id, question.get('question', '')[:500], json.dumps(question.get('options', [])),
             question.get('correct_index', 0), question.get('explanation', '')[:300], (topic or '')[:120]),
        )
        conn.commit()
        # Trim to the most recent MAX_WRONG_ANSWERS_PER_USER rows for this user.
        cur.execute(
            "DELETE FROM wrong_answers WHERE user_id=? AND id NOT IN ("
            "SELECT id FROM wrong_answers WHERE user_id=? ORDER BY id DESC LIMIT ?)",
            (user_id, user_id, MAX_WRONG_ANSWERS_PER_USER),
        )
        conn.commit()
    except Exception as e:
        logger.warning(f"log_wrong_answer failed: {e}")
    finally:
        conn.close()


def get_wrong_answers(user_id: int, limit: int = 20) -> list:
    conn = db_connect()
    try:
        cur = conn.cursor()
        cur.execute(
            "SELECT question, options, correct_index, explanation FROM wrong_answers "
            "WHERE user_id=? ORDER BY id DESC LIMIT ?",
            (user_id, limit),
        )
        rows = cur.fetchall()
        return [{"question": r[0], "options": json.loads(r[1]), "correct_index": r[2], "explanation": r[3]}
                for r in rows]
    finally:
        conn.close()
...
async def revise_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """/revise — rebuilds a fresh quiz from a user's own past wrong answers, across
    every quiz they've ever played, and sends it as a normal set of native quiz
    polls right here (no saved-quiz/DB row needed — it's built and sent on the fly)."""
    user_id = update.effective_user.id
    mistakes = get_wrong_answers(user_id, limit=20)
    if not mistakes:
        await update.message.reply_text(
            "🎯 No mistakes on record yet — play a few quizzes first, then come back "
            "here to revise anything you got wrong!"
        )
        return
    await update.message.reply_text(
        f"{ui_header('Revise My Mistakes', '🎯')}\nSending your last **{len(mistakes)}** wrong answers "
        f"as a fresh quiz — good luck this time! 💪",
        parse_mode="Markdown",
    )
    await post_quiz_polls(context.bot, update.effective_chat.id, mistakes)


# ==========================================
# ❓ LIVE DOUBT BUTTON (DM live quizzes only — see the send-poll site for why)
# ==========================================
MAX_DOUBT_MAP_SIZE = 300  # capped + oldest-evicted so this can never grow unbounded


def _remember_doubt_poll(application, poll_id: str, question: dict):
    doubt_map = application.bot_data.setdefault('doubt_map', OrderedDict())
    doubt_map[poll_id] = {
    ...
    doubt_map.move_to_end(poll_id)
    while len(doubt_map) > MAX_DOUBT_MAP_SIZE:
        doubt_map.popitem(last=False)


async def doubt_button_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    poll_id = query.data.replace("doubtbtn_", "")
    doubt_map = context.application.bot_data.get('doubt_map', {})
    entry = doubt_map.get(poll_id)
    if not entry:
        await query.answer("This question's details have expired — try /ask instead.", show_alert=True)
        return
    await query.answer("Explaining…")
    correct_text = entry['options'][entry['correct_index']] if entry['options'] else ''
    doubt_context = (
        f"Question: {entry['question']}\nOptions: {', '.join(entry['options'])}\n"
        f"Correct answer: {correct_text}\nShort explanation already given: {entry['explanation']}"
    )
    await context.bot.send_chat_action(chat_id=query.message.chat_id, action="typing")
    try:
        answer = await asyncio.to_thread(
            answer_doubt_sync, update.effective_user.id,
            "Explain why this is the correct answer, in more detail than a one-line explanation.",
            doubt_context,
        )
        await context.bot.send_message(query.message.chat_id, f"💡 {answer}")
    except RuntimeError as e:
        await context.bot.send_message(query.message.chat_id, str(e), parse_mode="Markdown")
    except Exception:
        await context.bot.send_message(query.message.chat_id, "⚠️ Couldn't get a detailed explanation right now — try /ask instead.")
...
# 🤝 STUDY BUDDY MATCH
# ==========================================
STUDY_BUDDY_ACCURACY_WINDOW = 15  # match anyone within +/- this many accuracy percentage points


async def studybuddy_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """/studybuddy — opt-in only (nobody gets matched without asking). Joins a
    queue; if someone already waiting has a similar accuracy, both are matched
    immediately and notified. Otherwise you wait until the next person joins."""
    user_id = update.effective_user.id
    stats = get_user_profile_stats(user_id)
    if stats['quizzes_played'] == 0:
        await update.message.reply_text("🤝 Play a few quizzes first so there's an accuracy to match on!")
        return
    accuracy = stats['accuracy']
    conn = db_connect()
    try:
        cur = conn.cursor()
        cur.execute("DELETE FROM study_buddy_queue WHERE user_id=?", (user_id,))  # re-joining refreshes your spot
        cur.execute(
            "SELECT user_id, user_name, accuracy FROM study_buddy_queue "
            "ORDER BY ABS(accuracy - ?) ASC LIMIT 1", (accuracy,),
        )
        candidate = cur.fetchone()
        if candidate and abs(candidate[2] - accuracy) <= STUDY_BUDDY_ACCURACY_WINDOW:
            other_id, other_name, other_acc = candidate
            cur.execute("DELETE FROM study_buddy_queue WHERE user_id=?", (other_id,))
            conn.commit()
            match_text = (
                f"{ui_header('Study Buddy Match!', '🤝')}\n"
                f"You're matched with **{other_name}** (accuracy: {other_acc}% vs your {accuracy}%).\n"
                f"Add each other and try a `/duel` — evenly matched, should be a good one!"
            )
            ...
            try:
                await context.bot.send_message(
                    other_id,
                    f"{ui_header('Study Buddy Match!', '🤝')}\n"
                    f"You're matched with **{update.effective_user.first_name}** "
                    f"(accuracy: {accuracy}% vs your {other_acc}%). Try a `/duel`!",
                    parse_mode="Markdown",
                )
            except Exception:
                pass
            ...
            cur.execute(
                "INSERT INTO study_buddy_queue (user_id, user_name, accuracy) VALUES (?, ?, ?)",
                (user_id, update.effective_user.first_name or "Player", accuracy),
            )
            conn.commit()
            await update.message.reply_text(
                f"🤝 You're in the queue (accuracy: {accuracy}%) — you'll get a DM the moment "
                f"someone similarly-skilled joins."
            )
    finally:
        conn.close()
...
# ==========================================
# ⏰ TIME CAPSULE QUIZ — a saved quiz that fires exactly once at a future time
# ==========================================
async def timecapsule_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """/timecapsule <quiz_id> <YYYY-MM-DD> <HH:MM> — schedules a ONE-TIME send of
    a saved quiz to THIS chat at that UTC date/time. Unlike /schedule (which
    repeats on an interval to every admin group), this fires once, to one chat,
    then is done — good for "unlock this on exam day" style reveals."""
    if len(context.args) < 3 or not context.args[0].isdigit():
        await update.message.reply_text(
            "Usage: `/timecapsule <quiz_id> <YYYY-MM-DD> <HH:MM>` (UTC time) — "
            "e.g. `/timecapsule 42 2026-12-25 09:00`",
            parse_mode="Markdown",
        )
        return
    quiz_id = int(context.args[0])
    if not get_quiz_by_id(quiz_id):
        await update.message.reply_text("⚠️ That quiz ID doesn't exist.")
        return
    try:
        run_at = datetime.strptime(f"{context.args[1]} {context.args[2]}", "%Y-%m-%d %H:%M")
    except ValueError:
        await update.message.reply_text("⚠️ Couldn't read that date/time — use `YYYY-MM-DD HH:MM` (UTC).", parse_mode="Markdown")
        return
    if run_at <= datetime.utcnow():
        await update.message.reply_text("⚠️ That time is in the past — pick a future date/time.")
        return
    conn = db_connect()
    try:
        cur = conn.cursor()
        cur.execute(
            "INSERT INTO time_capsules (quiz_id, chat_id, run_at, created_by) VALUES (?, ?, ?, ?)",
            (quiz_id, update.effective_chat.id, run_at.isoformat(), update.effective_user.id),
        )
        conn.commit()
    finally:
        conn.close()
    await update.message.reply_text(
        f"⏰ **Time Capsule set!** Quiz #{quiz_id} will unlock here at "
        f"**{run_at.strftime('%d %b %Y, %H:%M UTC')}**.",
        parse_mode="Markdown",
    )


async def time_capsule_check_job(context: ContextTypes.DEFAULT_TYPE):
    """Runs every minute; fires any time capsule whose moment has arrived. Mirrors
    the same due-row-polling pattern as scheduled_broadcast_check_job."""
    now_iso = datetime.utcnow().isoformat()
    conn = db_connect()
    cur = conn.cursor()
    cur.execute("SELECT id, quiz_id, chat_id, created_by FROM time_capsules WHERE sent=0 AND run_at<=?", (now_iso,))
    due = cur.fetchall()
    conn.close()
    for cap_id, quiz_id, chat_id, created_by in due:
        try:
            await context.bot.send_message(chat_id, "⏰ **A Time Capsule has unlocked!**", parse_mode="Markdown")
            await initiate_live_quiz(context.bot, context.application, chat_id, "group", quiz_id,
                                      created_by or OWNER_ID, "Time Capsule")
        except Exception as e:
            logger.warning(f"Time capsule #{cap_id} failed: {e}")
        conn = db_connect()
        cur = conn.cursor()
        cur.execute("UPDATE time_capsules SET sent=1 WHERE id=?", (cap_id,))
        conn.commit()
        conn.close()


async def battleroyale_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """/battleroyale <quiz_id> — group only. Runs like a normal live quiz (no
    waiting-room lobby, starts immediately) but the final leaderboard calls out
    who got a flawless run as a "Survivor". Telegram's poll API has no way to
    actually stop someone from tapping a poll option once they've gotten one
    wrong, so this is a scoring/labeling twist, not a real elimination block."""
    if update.effective_chat.type not in ("group", "supergroup"):
        await update.message.reply_text("⚔️ /battleroyale sirf group me chalta hai.")
        return
    if not context.args or not context.args[0].isdigit():
        await update.message.reply_text("Usage: `/battleroyale <quiz_id>`", parse_mode="Markdown")
        return
    quiz_id = int(context.args[0])
    await update.message.reply_text(
        f"{ui_header('Battle Royale', '⚔️')}\nOne wrong answer and you're marked eliminated on the "
        f"final board — only a flawless run survives! Starting now...",
        parse_mode="Markdown",
    )
    await start_live_quiz(context.bot, context.application, update.effective_chat.id, quiz_id,
                           update.effective_user.id, update.effective_user.first_name or "Host",
                           battle_royale=True)
...
async def do_regenerate(bot, chat_id, user_id):
    ...
    try:
        questions = await generate_mcqs_async(user_id, last['parts'], count, last['difficulty'], last['language'])
    except RuntimeError as e:
...
# ==========================================
# 👤 /profile — personal stats, streak, and badges (any chat, always about the
# caller themselves — cross-group, all-time, from the same quiz_results table
# /leaderboard and /topusers already use).
# ==========================================
BADGE_THRESHOLDS = [
    (1000, "💎 Legend — 1000+ correct answers"),
    (500, "🏆 Champion — 500+ correct answers"),
    (200, "🥇 Expert — 200+ correct answers"),
    (50, "🥈 Regular — 50+ correct answers"),
    (10, "🥉 Getting Started — 10+ correct answers"),
]
...
def get_user_profile_stats(user_id: int) -> dict:
    ...
    cur = conn.cursor()
    cur.execute(
        "SELECT COUNT(*), SUM(correct_count), SUM(wrong_count), SUM(total_questions) "
        "FROM quiz_results WHERE user_id=?", (user_id,)
    )
    row = cur.fetchone()
    quizzes_played = row[0] or 0
    correct = row[1] or 0
    wrong = row[2] or 0
    total_q = row[3] or 0
    cur.execute("SELECT streak_count, question_limit, questions_used, streak_freeze_count FROM users WHERE user_id=?", (user_id,))
    urow = cur.fetchone()
    conn.close()
    streak = (urow[0] or 0) if urow else 0
    freezes = (urow[3] or 0) if urow else 0
    total = correct + wrong
    accuracy = round((correct / total) * 100, 1) if total else 0.0
    badges = [label for threshold, label in BADGE_THRESHOLDS if correct >= threshold][:1]
    badges += [label for threshold, label in STREAK_BADGE_THRESHOLDS if streak >= threshold][:1]
    return {
        "quizzes_played": quizzes_played, "correct": correct, "wrong": wrong,
        "total_questions": total_q, "accuracy": accuracy, "streak": streak, "badges": badges,
...
async def profile_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    stats = get_user_profile_stats(target_user.id)
    daily_left = get_daily_ai_remaining(target_user.id)
    limit_line = "♾️ Unlimited" if daily_left is None else f"{daily_left}/{DAILY_AI_QUESTION_LIMIT}"
    badges_text = "  ".join(stats["badges"]) if stats["badges"] else "_None yet — keep playing quizzes to earn one!_"
    text = (
        f"👤 **{target_user.first_name}'s Profile**\n\n"
        f"🎯 Quizzes played: {stats['quizzes_played']}\n"
        f"✅ Correct: {stats['correct']}  ❌ Wrong: {stats['wrong']}\n"
        f"📊 Accuracy: {stats['accuracy']}%\n"
        f"🔥 Current streak: {stats['streak']} day(s)  ❄️ Freeze tokens: {stats['streak_freezes']}\n"
        f"🎟️ AI quota left today: {limit_line}\n\n"
        f"🏅 **Badges:** {badges_text}\n\n"
        f"_Tip: play any Official Quiz or group AI quiz to build your streak — one finished "
        f"quiz per UTC day keeps it going._"
    )
    await update.message.reply_text(text, parse_mode="Markdown")
...
async def topusers_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not top:
        ...
        return
    medals = ["🥇", "🥈", "🥉"] + ["▫️"] * 7
    lines = [f"{ui_header('Top 10 — Most Questions Solved', '🏆')}\n"]
...
def build_leaderboard_text(chat_id, mode: str = "points") -> str:
    ...
    if not rows:
        return f"{ui_header(title, '📊')}\n\nNo quiz results yet here."

    medals = ["🥇", "🥈", "🥉"]
    lines = [f"{ui_header(title, '📊')}\n"]
...
# ==========================================
# 🤺 /duel — 1v1 challenge. Reply to your opponent's message with
# /duel <topic> to start a short 5-question head-to-head, run through the exact
# same live-quiz engine as Official Quiz (so timers/pausing/etc. all just work),
# with a head-to-head result posted once it finishes.
# ==========================================
DUEL_QUESTION_COUNT = 5
DUEL_TIMER_SECONDS = 15


def adjust_coins(user_id: int, delta: int):
    """Positive delta credits, negative debits — never lets a balance go below 0."""
    conn = db_connect()
    try:
        cur = conn.cursor()
...
async def duel_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if update.effective_chat.type not in ("group", "supergroup"):
        await update.message.reply_text("🤺 /duel sirf group me chalta hai — kisi group me apne opponent ke message par reply karke try karo.")
        return
    replied = update.message.reply_to_message
    if not replied or not replied.from_user:
        await update.message.reply_text(
            "🤺 **Duel kaise shuru karein:**\nApne opponent ke kisi message par **reply** karke likho:\n"
            "`/duel <topic>` — jaise `/duel World Capitals`\n"
            "`/duel <topic> <coins>` — jaise `/duel World Capitals 50` to wager coins (winner takes both)",
            parse_mode="Markdown",
        )
        return
    ...
    if opponent.id == challenger.id:
        await update.message.reply_text("🤺 Khud se duel nahi ho sakta — kisi aur ko challenge karo!")
        return
    if opponent.is_bot:
        await update.message.reply_text("🤺 Bots ke saath duel nahi ho sakta.")
        return
    ...
    if args and args[-1].isdigit():
        wager = int(args.pop())
    topic = " ".join(args).strip() if args else "General Knowledge"

    if wager > 0:
        c_coins = get_user(challenger.id)['coins']
        o_coins = get_user(opponent.id)['coins']
        if c_coins < wager or o_coins < wager:
            await update.message.reply_text(
                f"🪙 Wager cancelled — someone doesn't have {wager} coins "
                f"({challenger.first_name}: {c_coins}, {opponent.first_name}: {o_coins}). Duel will run without a wager.",
            )
            wager = 0

    wager_line = f"\n🪙 Wager: **{wager} coins each** — winner takes both!" if wager else ""
    await update.message.reply_text(
        f"🤺 **Duel Challenge!**\n{challenger.first_name} vs {opponent.first_name}\n"
        f"Topic: {topic}\nGenerating {DUEL_QUESTION_COUNT} questions...{wager_line}",
        parse_mode="Markdown",
    )
    try:
        questions = await generate_mcqs_async(challenger.id, [{"type": "text", "text": topic}],
                                               DUEL_QUESTION_COUNT, "medium", "English")
    except RuntimeError as e:
        await report_generation_failure(context.bot, update.effective_chat.id, challenger.id, e)
        return

    quiz_id = save_quiz(challenger.id, f"Duel: {topic}"[:60], questions, timer_seconds=DUEL_TIMER_SECONDS,
                         quiz_type="duel", creator_name=challenger.first_name or "Unknown")

    conn = db_connect()
    cur = conn.cursor()
    cur.execute(
        "INSERT INTO duels (chat_id, quiz_id, challenger_id, challenger_name, opponent_id, opponent_name, status, wager) "
        "VALUES (?, ?, ?, ?, ?, ?, 'pending', ?)",
        (update.effective_chat.id, quiz_id, challenger.id, challenger.first_name or "Unknown",
         opponent.id, opponent.first_name or "Unknown", wager)
    )
    conn.commit()
    duel_id = last_insert_id(cur, conn, "duels")
    conn.close()

    await initiate_live_quiz(context.bot, context.application, update.effective_chat.id, update.effective_chat.type,
                              quiz_id, challenger.id, challenger.first_name or "Host")

    context.application.job_queue.run_repeating(
        _watch_duel_finish, interval=10, first=10,
        data={"duel_id": duel_id, "chat_id": update.effective_chat.id, "quiz_id": quiz_id, "tries": 0},
        name=f"duel_watch_{duel_id}",
    )


async def _watch_duel_finish(context: ContextTypes.DEFAULT_TYPE):
    """Polls the in-memory live-quiz state (population happens in start_live_quiz /
    _run_live_quiz_loop) until this duel's session is finished, then posts a
    head-to-head comparison. Gives up after ~10 minutes so a duel nobody starts
    (lobby never fills) doesn't leave a job running forever."""
    job = context.job
    data = job.data
    data["tries"] += 1
    live = context.application.bot_data.get("live_quizzes", {})
    session = live.get(data["chat_id"])
    session_matches = session and session.get("quiz_id") == data["quiz_id"]
    finished = (not session_matches) or session.get("finished", False)
    if not finished and data["tries"] < 60:
        return  # keep polling — still running or lobby hasn't started it yet

    job.schedule_removal()

    conn = db_connect()
    cur = conn.cursor()
    cur.execute("SELECT challenger_id, challenger_name, opponent_id, opponent_name, status, wager FROM duels WHERE id=?",
                (data["duel_id"],))
    row = cur.fetchone()
    if not row or row[4] == "finished":
        conn.close()
        return
    challenger_id, challenger_name, opponent_id, opponent_name, _, wager = row
    wager = wager or 0

    def _score_for(uid):
        cur.execute("SELECT correct_count, wrong_count FROM quiz_results WHERE quiz_id=? AND user_id=?",
                    (data["quiz_id"], uid))
        r = cur.fetchone()
        return (r[0] or 0, r[1] or 0) if r else (0, 0)

    c_correct, c_wrong = _score_for(challenger_id)
    o_correct, o_wrong = _score_for(opponent_id)
    cur.execute("UPDATE duels SET status='finished' WHERE id=?", (data["duel_id"],))
    conn.commit()
    conn.close()

    if c_correct == 0 and c_wrong == 0 and o_correct == 0 and o_wrong == 0:
        return  # nobody answered at all (lobby likely never filled) — nothing worth posting

    wager_line = ""
    if c_correct > o_correct:
        verdict = f"🏆 **{challenger_name} wins the duel!**"
        if wager:
            adjust_coins(challenger_id, wager)
            adjust_coins(opponent_id, -wager)
            wager_line = f"\n🪙 {challenger_name} wins **{wager} coins** from {opponent_name}!"
    elif o_correct > c_correct:
        verdict = f"🏆 **{opponent_name} wins the duel!**"
        if wager:
            adjust_coins(opponent_id, wager)
            adjust_coins(challenger_id, -wager)
            wager_line = f"\n🪙 {opponent_name} wins **{wager} coins** from {challenger_name}!"
    else:
        verdict = "🤝 **It's a tie!**"
        if wager:
            wager_line = "\n🪙 Tie — no coins change hands."

    await context.bot.send_message(
        data["chat_id"],
        f"🤺 **Duel Result**\n\n"
        f"{challenger_name}: {c_correct}✅ / {c_wrong}❌\n"
        f"{opponent_name}: {o_correct}✅ / {o_wrong}❌\n\n{verdict}{wager_line}",
        parse_mode="Markdown",
    )
...
async def browse_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """/browse [keyword] — search your own saved quizzes by title. No keyword lists
    ...
    if keyword:
        cur.execute(
            "SELECT id, title, quiz_type, timer_seconds FROM quizzes WHERE user_id=? AND title LIKE ? "
            "ORDER BY id DESC LIMIT 25",
            (user_id, f"%{keyword}%")
        )
    else:
        cur.execute(
            "SELECT id, title, quiz_type, timer_seconds FROM quizzes WHERE user_id=? ORDER BY id DESC LIMIT 25",
            (user_id,)
        )
    rows = cur.fetchall()
    conn.close()
    if not rows:
        msg = f"No saved quizzes matching '{keyword}'." if keyword else "You haven't saved any quizzes yet."
        await update.message.reply_text(msg)
        return
    header = f"🔎 **Results for '{keyword}'**" if keyword else "📚 **Your Saved Quizzes** (most recent first)"
    lines = [header]
    for r in rows:
        timer_text = f"{r[3]}s/q" if r[3] else "no timer"
        lines.append(f"`{r[0]}` — {r[1]} _({r[2]}, {timer_text})_")
    lines.append("\n_Play any of these with_ `/startquiz <id>` _· export with_ `/exportquiz <id>` _or_ `/exportresults <id>`")
    await update.message.reply_text("\n".join(lines), parse_mode="Markdown")


async def exportquiz_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """/exportquiz <id> — sends the quiz's questions as a downloadable JSON file
    (owner's own quiz, or bot owner for any quiz)."""
    if not context.args or not context.args[0].isdigit():
        await update.message.reply_text("Usage: `/exportquiz <quiz id>` (see IDs with `/browse`)", parse_mode="Markdown")
        return
    quiz = get_quiz_meta(int(context.args[0]))
    if not quiz or (quiz["user_id"] != update.effective_user.id and update.effective_user.id != OWNER_ID):
        await update.message.reply_text("❌ Quiz not found, or it isn't yours.")
        return
    payload = {"title": quiz["title"], "quiz_type": quiz["quiz_type"],
               "timer_seconds": quiz["timer_seconds"], "questions": quiz["questions"]}
    buf = io.BytesIO(json.dumps(payload, ensure_ascii=False, indent=2).encode("utf-8"))
    buf.name = f"quiz_{quiz['id']}.json"
    await update.message.reply_document(buf, filename=buf.name, caption=f"📤 Exported: {quiz['title']}")


async def exportresults_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """/exportresults <id> — CSV of everyone who played this quiz and how they did
    (owner's own quiz, or bot owner for any quiz)."""
    if not context.args or not context.args[0].isdigit():
        await update.message.reply_text("Usage: `/exportresults <quiz id>` (see IDs with `/browse`)", parse_mode="Markdown")
        return
    quiz_id = int(context.args[0])
    quiz = get_quiz_meta(quiz_id)
    if not quiz or (quiz["user_id"] != update.effective_user.id and update.effective_user.id != OWNER_ID):
    ...
    cur = conn.cursor()
    cur.execute(
        "SELECT user_id, user_name, correct_count, wrong_count, total_questions, score, avg_time_ms, finished_at "
        "FROM quiz_results WHERE quiz_id=? ORDER BY finished_at", (quiz_id,)
    )
    rows = cur.fetchall()
    conn.close()
    if not rows:
        await update.message.reply_text("No results yet for this quiz — nobody has played it live.")
        return
    text_buf = io.StringIO()
    writer = csv.writer(text_buf)
    writer.writerow(["user_id", "user_name", "correct", "wrong", "total_questions", "score", "avg_time_ms", "finished_at"])
    writer.writerows(rows)
    buf = io.BytesIO(text_buf.getvalue().encode("utf-8"))
    buf.name = f"quiz_{quiz_id}_results.csv"
    await update.message.reply_document(buf, filename=buf.name, caption=f"📊 Results export: {quiz['title']}")
...
def _store_manage_view(quiz_id: int):
    ...
    keyboard_rows = [
        ...
        [InlineKeyboardButton("💎 Poll Style (Native/Pro)", callback_data="pollstylemenu")],
...
async def public_quiz_view_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    keyboard = InlineKeyboardMarkup([
        [InlineKeyboardButton("▶️ Start Here", callback_data=f"oqstart_{quiz_id}")],
        [InlineKeyboardButton("📥 Clone to My Store (Advance Zone)", callback_data=f"clonequiz_{quiz_id}")],
        [InlineKeyboardButton("🔙 Back to Public Store", callback_data="publist_0")],
    ])
    await query.edit_message_text(text, parse_mode="Markdown", reply_markup=keyboard)


async def clone_quiz_to_my_store(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Advance Zone entry point: copies a public/official quiz's questions into
    a brand-new quizzes row owned by the CALLER — a real, independent copy,
    not a shared reference, so editing/deleting it never touches the
    original. Because ownership is what decides whose tag/description/promo
    link/option-style gets applied when a quiz is sent out (see
    store_send_channel_do above), the clone instantly becomes editable and
    postable through every tool /mystore already has — no new UI needed."""
    query = update.callback_query
    await query.answer("Cloning...")
    quiz_id = int(query.data.replace("clonequiz_", ""))
    source = get_quiz_meta(quiz_id)
    ...
    new_id = save_quiz(
        ...
        creator_name=caller.first_name or "",
        description=source["description"],
    )
    await query.edit_message_text(
        f"{ui_header('Cloned to Your Store', '📥')}\n\n"
        f"📁 **{source['title']}** ({len(source['questions'])} questions) is now in your own /mystore — "
        f"editing or deleting it will never affect the original.\n\n"
        f"▸ Set `/tag` — your own tag shown on every question\n"
        f"▸ Edit **Description** — your custom blurb / promotion link, from /mystore → this quiz\n"
        f"▸ **Option Style** — from /mystore → ⚙️ Personal Settings\n"
        f"▸ **Send to Channel** — from /mystore → this quiz → 📤 Send to Channel\n\n"
        f"All four apply automatically the moment you post it — nothing else to configure.",
        parse_mode="Markdown",
        reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("📁 Open My Store", callback_data=f"stmanage_{new_id}")]]),
    )
...
SKIP_API_MESSAGE = (
    ...
    "Bina kisi key ke bhi tum seedha /topic, /pdf, /image ya /ask "
...
async def limits_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """/limits — shows today's tracked usage vs. daily cap for every key in this
    user's fallback chain (their own /addapi keys + the owner's shared key)."""
    user_id = update.effective_user.id
    chain = build_provider_chain(user_id)
    today = datetime.utcnow().strftime("%Y-%m-%d")
    conn = db_connect()
    cur = conn.cursor()
    lines = ["📊 **Today's API usage** (UTC date, resets at 00:00 UTC):"]
    for cand in chain:
        limit = get_provider_daily_limit(cand["provider"])
        scope = _usage_scope(cand)
        cur.execute("SELECT count FROM api_key_usage WHERE scope=? AND usage_date=?", (scope, today))
        row = cur.fetchone()
        used = row[0] if row else 0
        if limit:
            pct = int(used * 100 / limit)
            bar = "🟥" if pct >= 90 else ("🟨" if pct >= 50 else "🟩")
            lines.append(f"{bar} {cand['label']} — {used}/{limit} ({pct}%)")
        else:
            lines.append(f"⬜ {cand['label']} — {used} calls today (no fixed cap tracked)")
    conn.close()
    lines.append(
        "\n_Ye published free-tier caps par based estimates hain (provider apni limits "
        "kabhi bhi badal sakta hai) — asli, live numbers ke liye hamesha provider ke apne "
        "dashboard ko check karo. Bot ka counter hamesha UTC midnight par reset hota hai._"
    )
    await update.message.reply_text("\n".join(lines), parse_mode="Markdown")
...
async def myschedules_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not schedules:
        ...
        return
    now = datetime.utcnow()
    lines = [f"{ui_header('Scheduled Broadcasts', '📅')}\n"]
    keyboard = []
    for s in schedules:
...
async def button_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "menu_main":
        ...
        await query.edit_message_text(
            text=f"{ui_header('Tools & Features Menu', '🛠️')}\n\nSelect any tool below to proceed:",
            parse_mode="Markdown",
        ...
        await query.edit_message_text(
            text=(
                f"{ui_header('Group Management Panel', '🛡️')}\n\n"
...
class _KeepAliveServer(ThreadingHTTPServer):
    daemon_threads = True
    # Every accepted connection gets its own thread (ThreadingHTTPServer), so one
    # slow/hanging client (a laggy UptimeRobot check, a browser left open, etc.)
    # can never block later health-check pings behind it — the old plain
    # HTTPServer was single-threaded, so exactly that could silently wedge the
    # whole health endpoint while the bot itself kept running fine (which is
    # why the Telegram-side logs looked perfectly healthy while UptimeRobot
    # still reported the service as Down).
    timeout = 30  # a stuck request is dropped instead of blocking forever
...
def start_keepalive_server():
    ...
    def _serve():
        # SAFETY NET: serve_forever() used to run unsupervised in a bare daemon
        # thread — if it ever raised (bad socket state, a dropped connection at
        # just the wrong moment, etc.) the thread died silently and the health
        # endpoint went dark PERMANENTLY for the rest of the process's life,
        # even though the Telegram bot itself kept running completely normally.
        # That produced exactly the reported symptom: clean, healthy logs, but
        # UptimeRobot stuck on "Down" until the next redeploy. This loop
        # rebuilds and restarts the server instead of letting one bad moment
        # kill it for good.
        while True:
            try:
                server = _KeepAliveServer(("0.0.0.0", port), _PingHandler)
                server.serve_forever()
            except Exception as e:
                logger.error(f"Keep-alive server crashed, restarting in 2s: {e}")
                time.sleep(2)
...
async def settings_compat(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.effective_message.reply_text(
        f"{ui_header('Personal Settings', '⚙️')}\n\n"
...
async def stats_compat(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    cur.execute("SELECT COUNT(*), COALESCE(SUM(correct_count+wrong_count),0) FROM quiz_results")
    results, total_answers = cur.fetchone()
    cur.execute("SELECT COUNT(*) FROM user_api_keys"); user_keys = cur.fetchone()[0]
    cur.execute("SELECT COUNT(*) FROM daily_quiz_subs"); daily_subs = cur.fetchone()[0]
    cur.execute("SELECT COUNT(*) FROM feedback"); feedback_count = cur.fetchone()[0]
    conn.close()
    await update.effective_message.reply_text(
        "📊 **BOT STATISTICS**\n\n"
        f"👤 Users: {users}\n📚 Saved quizzes: {quizzes}\n"
        f"👥 Tracked groups: {groups}\n🏆 Result records: {results}  (total answers: {total_answers})\n"
        f"🔑 Personal API keys added: {user_keys}\n☀️ Daily quiz subscriptions: {daily_subs}\n"
        f"💬 Feedback received: {feedback_count}", parse_mode="Markdown"
    )
...
def main():
    ...
    async def _post_init(application):
        # Default executor is only min(32, cpu_count+4) workers — too small once
        # several users are generating AI quizzes (each now running several
        # concurrent batches, see generate_mcqs_async) at the same time. A bigger
        # pool means one user's AI generation is far less likely to make everyone
        # else's commands/polls sit and wait for a free thread.
    ...
    app.add_handler(CommandHandler("undoquiz", undoquiz_command))
    app.add_handler(CommandHandler("fastquiz", fastquiz_command))
    app.add_handler(CommandHandler("slowquiz", slowquiz_command))
    ...
    ai_conv = ConversationHandler(
        ...
        states={
            ...
            ASK_AI_TIMER: [CallbackQueryHandler(receive_ai_timer, pattern="^aitimer_")],
            ASK_AI_SHUFFLE: [CallbackQueryHandler(receive_ai_shuffle, pattern="^aishuffle_")],
    ...
    # --- Quiz Import & Customize conversation ---
    importquiz_conv = ConversationHandler(
        entry_points=[CommandHandler("importquiz", importquiz_command)],
        states={
            IMPORT_QUIZ_POLL: [MessageHandler(filters.POLL, receive_importquiz_poll)],
            IMPORT_QUIZ_TAG: [MessageHandler(filters.TEXT & ~filters.COMMAND, receive_importquiz_tag)],
            IMPORT_QUIZ_DESC: [MessageHandler(filters.TEXT & ~filters.COMMAND, receive_importquiz_desc)],
            IMPORT_QUIZ_PROMO: [MessageHandler(filters.TEXT & ~filters.COMMAND, receive_importquiz_promo)],
        },
        fallbacks=[
            CommandHandler("cancel", cancel_command),
            CommandHandler("stop", cancel_command),
    ...
    app.add_handler(importquiz_conv)
    ...
    app.add_handler(CallbackQueryHandler(lobby_startnow_callback, pattern="^oqstartnow_"))
    ...
    app.add_handler(CommandHandler(["profile", "mystats"], profile_command))
    app.add_handler(CommandHandler("duel", duel_command))
    app.add_handler(CommandHandler("dailyquiz", dailyquiz_command))
    ...
    app.add_handler(CallbackQueryHandler(clone_quiz_to_my_store, pattern="^clonequiz_\\d"))
    ...
    app.add_handler(CallbackQueryHandler(poll_style_settings_callback, pattern="^pollstylemenu$"))
    app.add_handler(CallbackQueryHandler(poll_style_set_callback, pattern="^pollstyle_(native|pro)$"))

    app.add_handler(CommandHandler("krish", krish_command))
    app.add_handler(CommandHandler("refer", refer_command))
    app.add_handler(CommandHandler("revise", revise_command))
    app.add_handler(CallbackQueryHandler(doubt_button_callback, pattern="^doubtbtn_"))
    ...
    app.add_handler(CommandHandler("jointournament", join_tournament_command))
    app.add_handler(CommandHandler("battleroyale", battleroyale_command))
    app.add_handler(CommandHandler("studybuddy", studybuddy_command))
    app.add_handler(CommandHandler("timecapsule", timecapsule_command))
    app.add_handler(CallbackQueryHandler(refer_menu_callback, pattern="^refer_menu$"))
    app.add_handler(CallbackQueryHandler(krish_menu_callback, pattern="^krish_menu$"))
    app.add_handler(CallbackQueryHandler(krish_ping_callback, pattern="^krish_ping$"))
    app.add_handler(CallbackQueryHandler(krish_meet_owner_callback, pattern="^krish_meet_owner$"))
    app.add_handler(CallbackQueryHandler(krish_advance_callback, pattern="^krish_advance$"))
    # Runs before other text/sticker handling and is a no-op unless the relevant
    # Krish Mode flag is set on this user, so it can't interfere with anything else.
    app.add_handler(MessageHandler(filters.TEXT & ~filters.COMMAND, krish_receive_owner_message), group=-1)
    app.add_handler(MessageHandler(filters.Sticker.ALL, handle_advance_sticker), group=-1)
    ...
    app.add_handler(CommandHandler("exportquiz", exportquiz_command))
    app.add_handler(CommandHandler("exportresults", exportresults_command))
    ...
    # --- Owner: AI Mode (auto AI quiz polls + daily leaderboard, every synced group) ---
    ...
    if app.job_queue:
        ...
        app.job_queue.run_repeating(daily_quiz_check_job, interval=3600, first=60, name="daily_quiz_check")
        app.job_queue.run_repeating(scheduled_broadcast_check_job, interval=60, first=45, name="scheduled_broadcast_check")
        app.job_queue.run_repeating(time_capsule_check_job, interval=60, first=50, name="time_capsule_check")
        # AI Mode: checked every 5 sec so a custom `interval_seconds` as low as ~30s
        # (see /aimode on) still fires close to on time; the leaderboard only needs
        # to fire once/day so it reuses the same hourly cadence as daily_quiz_check.
        app.job_queue.run_repeating(ai_mode_quiz_check_job, interval=5, first=10, name="ai_mode_quiz_check")
        app.job_queue.run_repeating(ai_mode_leaderboard_job, interval=3600, first=75, name="ai_mode_leaderboard_check")
...
def _notify_owner_of_crash(exc: BaseException):
    """Best-effort DM to the owner with exactly what just crashed the process, so
    'why does it keep stopping' has a real, specific answer waiting in Telegram
    instead of living only in server logs the owner likely never opens. This is
    the ONLY place a crash gets reported like this — error_handler above only
    catches problems inside a single update/job (those never take the bot down
    at all); this covers the process-level kind that actually does. Swallows its
    own failures silently — it must never block the restart loop itself (e.g. if
    the crash WAS a total loss of network, this DM will also fail to send, and
    that's fine, restart still proceeds either way)."""
    if not OWNER_ID or not BOT_TOKEN:
        return
    try:
        tail = "".join(traceback.format_exception(type(exc), exc, exc.__traceback__))[-600:]

        async def _send():
            bot = Bot(token=BOT_TOKEN)
            await bot.send_message(
                OWNER_ID,
                f"🔴 **Bot process crashed — restarting now.**\n\n"
                f"`{type(exc).__name__}: {str(exc)[:200]}`\n\n"
                f"```\n{tail}\n```\n\n"
                f"Agar ye BAAR BAAR aa raha hai (same error), ye wahi cheez hai jo "
                f"'24×7 chalu nahi rehti' ka असल reason hai — sirf ek baar aana normal hai "
                f"(host kabhi-kabhi restart karta hai), lekin repeat hona matlab kuch "
                f"actually crash ho raha hai jise fix karna padega.",
                parse_mode="Markdown",
            )
        asyncio.run(_send())
    except Exception:
        pass  # never let the crash-notifier itself break the restart loop
...
def run_forever():
    ...
    logged with its full traceback AND DMed to the owner (see _notify_owner_of_crash)
    so the real cause is visible somewhere it'll actually get seen.

    If the bot is going offline repeatedly and this loop is why, the two other most
    likely causes (both external to this code, see the KEEP-ALIVE WEB SERVER comment
    near start_keepalive_server) are: (1) no uptime pinger is actually configured
    against the keep-alive URL yet, so a free host's inactivity sleep still kicks in
    between pings, or (2) a second copy of this exact bot is running somewhere else
    with the same BOT_TOKEN (error_handler's Conflict branch DMs the owner about that
    one specifically)."""
    backoff = 5
    while True:
        try:

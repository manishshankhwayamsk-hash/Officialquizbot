...
def init_db():
    ...
    # Pending /schedule jobs — a saved quiz + a target time. The row lives in the DB
    # (not just the in-memory job_queue job), so rearm_pending_schedules() can put it
    # back on the job_queue after a restart/redeploy instead of it just vanishing.
    cursor.execute(f"""
    ...
    # Running scores for an Official Quiz that is CURRENTLY live — updated after every
    # poll answer (not just once at the end). If the process restarts/redeploys mid-quiz,
    # whatever was earned before that point is not lost: recover_interrupted_live_quizzes()
    # finds these leftover rows on startup and finalizes them into quiz_results instead of
    # silently discarding that chat's in-progress leaderboard.
    cursor.execute(f"""
        CREATE TABLE IF NOT EXISTS live_quiz_progress (
            chat_id INTEGER,
            user_id INTEGER,
            user_name TEXT DEFAULT '',
            quiz_id INTEGER,
...
def run_migrations():
    ...
    new_columns = (
        ...
        ("quizzes", "description", "TEXT DEFAULT ''"),
        ("scheduled_quizzes", "user_name", "TEXT DEFAULT ''"),
        ("scheduled_quizzes", "chat_type", "TEXT DEFAULT ''"),
        ("scheduled_quizzes", "quiz_id", "INTEGER DEFAULT NULL"),
        ("scheduled_quizzes", "quiz_title", "TEXT DEFAULT ''"),
    )
...
def get_quiz_meta(quiz_id: int):
    ...
    cur = conn.cursor()
    cur.execute(
        "SELECT id, user_id, title, data, timer_seconds, quiz_type, creator_name, description, set_id "
    ...
    return {
        ...
        "description": row[7] or "", "set_id": row[8],
    }
...
def save_live_quiz_progress(chat_id: int, quiz_id: int, quiz_title: str, total_questions: int, entry: dict):
    """Upserts one player's running score for a currently-live Official Quiz, so a
    mid-quiz restart/redeploy doesn't wipe points nobody has 'finished' earning yet."""
    conn = db_connect()
    cur = conn.cursor()
    upsert(
        cur, "live_quiz_progress",
        {"chat_id": chat_id, "user_id": entry["user_id"]},
        {"user_name": entry["name"][:60], "quiz_id": quiz_id, "quiz_title": quiz_title[:120],
         "total_questions": total_questions, "score": entry["score"], "correct_count": entry["correct"],
         "wrong_count": entry["wrong"], "total_time_ms": entry["total_time_ms"], "answered": entry["answered"]},
    )
    conn.commit()
    conn.close()


def clear_live_quiz_progress(chat_id: int):
    conn = db_connect()
    cur = conn.cursor()
    cur.execute("DELETE FROM live_quiz_progress WHERE chat_id=?", (chat_id,))
    conn.commit()
    conn.close()


def get_interrupted_live_quiz_chats() -> list:
    """Any chat_id still sitting in live_quiz_progress at startup means the previous
    process died (crash/redeploy) mid-quiz — that quiz was never finished/cleared."""
    conn = db_connect()
    cur = conn.cursor()
    cur.execute("SELECT DISTINCT chat_id FROM live_quiz_progress")
    rows = [r[0] for r in cur.fetchall()]
    conn.close()
    return rows


def get_live_quiz_progress_rows(chat_id: int) -> list:
    conn = db_connect()
    cur = conn.cursor()
    cur.execute(
        "SELECT user_id, user_name, quiz_id, quiz_title, total_questions, score, correct_count, "
        "wrong_count, total_time_ms, answered FROM live_quiz_progress WHERE chat_id=?",
        (chat_id,)
    )
    rows = cur.fetchall()
    conn.close()
    return [
        {"user_id": r[0], "name": r[1] or "Player", "quiz_id": r[2], "quiz_title": r[3] or "Quiz",
         "total_questions": r[4] or 0, "score": r[5] or 0, "correct": r[6] or 0, "wrong": r[7] or 0,
         "total_time_ms": r[8] or 0, "answered": r[9] or 0}
        for r in rows
    ]
    ...
    for cand in chain:
        try:
            if cand["provider"] == "anthropic":
                return _generate_via_anthropic(cand["api_key"], cand["model"], source_parts, count, difficulty, language)
            elif cand["provider"] == "gemini":
                return _generate_via_gemini(cand["api_key"], cand["model"], source_parts, count, difficulty, language)
            else:
                return _generate_via_openai_compatible(cand["api_key"], cand["api_base"], cand["model"], source_parts, count, difficulty, language)
...
async def help_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    help_text = (
        ...
        f"speed) · 📁 file in a Set · 📤 Send to Channel\n\n"
        f"📊 **Poll & Control:**\n"
        ...
        f"• `/addapi` — add Groq, Gemini, OpenAI, Anthropic, or any open-source/self-hosted key — add as many as you want\n"
        ...
        f"• `/schedule` — Schedule a resend of your last quiz\n"
        f"• `/myschedules` — View or cancel pending scheduled quizzes"
    )
    ...
    async def _entry(update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        if source_type == 'youtube' and context.args:
            ...
            try:
                ...
                await update.message.reply_text(f"Couldn't fetch that video's transcript: {e}")
                return ConversationHandler.END
...
async def receive_content(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if source_type == 'youtube':
        ...
        try:
            ...
            await msg.reply_text(f"Couldn't fetch that video's transcript: {e}")
            return ConversationHandler.END
        context.user_data['gen_parts'] = [{"type": "text", "text": transcript[:15000]}]
        return await ask_difficulty(update, context)
...
async def _finish_live_quiz(bot, chat_id, application):
    ...
    try:
        clear_live_quiz_progress(chat_id)
    except Exception as e:
        logger.warning(f"clear_live_quiz_progress failed (non-fatal): {e}")
    live.pop(chat_id, None)


async def recover_interrupted_live_quizzes(bot):
    """Runs once at startup. If any chat still has rows in live_quiz_progress, the
    previous process died (crash, redeploy, host restart) in the middle of an Official
    Quiz there. That quiz can't be safely resumed (the old poll message/timer is gone),
    but the points already earned are still on disk — so post the leaderboard for
    however far it got, save it into quiz_results (counted in /leaderboard, /topusers,
    etc.), and clear the leftover rows. This is what makes points survive a redeploy
    even when it happens mid-quiz, not just between quizzes."""
    try:
        chat_ids = get_interrupted_live_quiz_chats()
    except Exception as e:
        logger.warning(f"recover_interrupted_live_quizzes lookup failed: {e}")
        return
    for chat_id in chat_ids:
        rows = get_live_quiz_progress_rows(chat_id)
        if not rows:
            continue
        ...
        for i, r in enumerate(rows[:20]):
            accuracy = round((r["correct"] / r["answered"]) * 100, 1) if r["answered"] else 0.0
            rank_tag = medals[i] if i < 3 else f"{i + 1}."
            lines.append(f"{rank_tag} {r['name']} — `{r['user_id']}`\n"
                         f"      {r['score']} pts · ✅ {r['correct']} ❌ {r['wrong']} · 🎯 {accuracy}%")
        board_text = (
            f"🏆 **{stylize('Official Quiz Leaderboard')}** — {title}\n"
            "⚠️ This quiz was interrupted by a restart, so it was auto-finished — "
            "here's the leaderboard for what was played so far. Points below are already saved.\n\n"
            + "\n".join(lines)
        )
        try:
            await bot.send_message(chat_id, board_text, parse_mode="Markdown")
        except Exception as e:
            logger.warning(f"recover_interrupted_live_quizzes leaderboard send failed for {chat_id}: {e}")
        for r in rows:
            avg_ms = int(r["total_time_ms"] / r["answered"]) if r["answered"] else 0
            try:
                save_quiz_result(r["quiz_id"], chat_id, r["user_id"], r["name"], r["score"], r["correct"],
                                  wrong_count=r["wrong"], total_questions=r["total_questions"], avg_time_ms=avg_ms)
            except Exception as e:
                logger.warning(f"recover_interrupted_live_quizzes save failed for user {r['user_id']}: {e}")
        clear_live_quiz_progress(chat_id)
    if chat_ids:
        logger.info(f"Recovered {len(chat_ids)} interrupted live quiz(zes) after restart.")
...
async def handle_poll_answer(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not ans.user:
        return  # voted as an anonymous channel identity — can't be scored to a user_id
    ...
    for chat_id, session in live.items():
        if ans.poll_id in session['poll_map']:
            ...
            # Persist this player's running score immediately — not just at the end —
            # so a redeploy mid-quiz can't silently erase points already earned.
            try:
                save_live_quiz_progress(chat_id, session['quiz_id'], session['title'],
                                         len(session['questions']), entry)
            except Exception as e:
                logger.warning(f"save_live_quiz_progress failed (non-fatal): {e}")
            break
...
async def receive_api_custom_base(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not base.startswith("http"):
        await update.message.reply_text("That doesn't look like a URL — send the full base URL (starting with http/https).")
        return ASK_API_CUSTOM_BASE
    context.user_data['api_base'] = base.rstrip('/')
    await update.message.reply_text(
        "Now send the model name to use (e.g. `llama3.1`, `mixtral-8x7b-32768`, `qwen2.5:7b`).",
        parse_mode="Markdown",
    )
...
async def receive_api_custom_model(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        "Send the API key for this endpoint, or type `none` if it doesn't need one "
        "(e.g. a local server on your own machine).",
        parse_mode="Markdown",
    )
    return ASK_API_KEY
...
async def receive_api_key(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if provider != "custom" and not api_key:
        await update.message.reply_text("That key looks empty — please send a valid key, or /cancel.")
        return ASK_API_KEY
...
# ==========================================
# ⏰ SCHEDULE — pick a time, then pick a saved quiz. At that time every question in
# that quiz auto-broadcasts here exactly like a live Official Quiz (polls, timer,
# pause/resume/stop controls, and a final leaderboard with points saved) — not a
# plain re-send. The row lives in scheduled_quizzes (not just the in-memory
# job_queue job), and rearm_pending_schedules() re-arms every pending row on every
# startup, so a redeploy or restart before the scheduled time never loses it.
# ==========================================
SCHEDULE_TIME_PRESETS = [("15 min", 15), ("30 min", 30), ("1 hour", 60), ("3 hours", 180)]


async def schedule_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    # Backward-compatible shortcut: /schedule <minutes> skips straight to quiz-picking.
    if context.args and context.args[0].isdigit():
        return await _schedule_ask_quiz(update, context, int(context.args[0]))

    keyboard = [[InlineKeyboardButton(label, callback_data=f"schedtime_{mins}")]
                for label, mins in SCHEDULE_TIME_PRESETS]
    await update.message.reply_text(
        f"⏰ **{stylize('Schedule an Official Quiz')}**\n\n"
        "When should it auto-start? Pick a time below, or send /schedule <minutes> for a custom delay.",
...
async def schedule_time_button(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    await query.answer()
    minutes = int(query.data.replace("schedtime_", ""))
    await _schedule_ask_quiz(update, context, minutes, from_button=True)


async def _schedule_ask_quiz(update: Update, context: ContextTypes.DEFAULT_TYPE, minutes: int, from_button: bool = False):
    user_id = update.effective_user.id
    quizzes = get_user_quizzes(user_id)
    if not quizzes:
        text = "You don't have any saved quizzes yet — create one first (e.g. /topic, /pdf, /create)."
        if from_button:
    ...
    keyboard = [[InlineKeyboardButton(q["title"][:40] or f"Quiz #{q['id']}",
                                       callback_data=f"schedquiz_{minutes}_{q['id']}")] for q in quizzes[:20]]
    text = f"⏰ Starting in {minutes} minute(s) — which quiz should auto-broadcast here?"
    if from_button:
...
async def schedule_quiz_button(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    await query.answer()
    try:
        _, minutes_str, quiz_id_str = query.data.split("_", 2)
        minutes, quiz_id = int(minutes_str), int(quiz_id_str)
    except ValueError:
        await query.edit_message_text("❌ Something went wrong — please run /schedule again.")
        return
    ...
    cur = conn.cursor()
    cur.execute(
        "INSERT INTO scheduled_quizzes (user_id, user_name, chat_id, chat_type, quiz_id, quiz_title, run_at) "
        "VALUES (?, ?, ?, ?, ?, ?, ?)",
        (user.id, user.first_name or "Scheduler", chat.id, chat.type, quiz_id, quiz["title"][:120], run_at.isoformat()),
    )
    conn.commit()
    ...
    _arm_schedule_job(
        context.application, schedule_id, chat.id, chat.type, quiz_id,
        user.id, user.first_name or "Scheduler", minutes * 60,
    )
    await query.edit_message_text(
        f"⏰ **{quiz['title']}** will auto-start here in {minutes} minute(s) (#{schedule_id}).\n"
        "It'll run exactly like a live Official Quiz — polls, timer, controls, and "
        "leaderboard/points. Check /myschedules to view or cancel it.",
        parse_mode="Markdown",
    )


def _arm_schedule_job(application, schedule_id, chat_id, chat_type, quiz_id, user_id, user_name, delay_seconds):
    application.job_queue.run_once(
        run_scheduled_quiz, max(0, delay_seconds),
        data={"schedule_id": schedule_id, "chat_id": chat_id, "chat_type": chat_type,
              "quiz_id": quiz_id, "user_id": user_id, "user_name": user_name},
        name=f"sched_{schedule_id}",
    )


async def run_scheduled_quiz(context: ContextTypes.DEFAULT_TYPE):
    data = context.job.data
    conn = db_connect()
    cur = conn.cursor()
    cur.execute("DELETE FROM scheduled_quizzes WHERE id = ?", (data["schedule_id"],))
    conn.commit()
    conn.close()
    try:
        await initiate_live_quiz(
            context.bot, context.application, data["chat_id"], data.get("chat_type") or "private",
            data["quiz_id"], data["user_id"], data.get("user_name") or "Scheduler",
        )
    except Exception as e:
        logger.warning(f"Scheduled quiz #{data.get('schedule_id')} failed to run: {e}")


def rearm_pending_schedules(application):
    """Called once at startup. job_queue jobs are in-memory only, so a restart/redeploy
    always drops them — but the row in scheduled_quizzes survives, and this puts every
    pending one straight back on the job_queue. An overdue row (bot was down past its
    run_at) fires almost immediately instead of being silently lost."""
    if not application.job_queue:
        return
    conn = db_connect()
    cur = conn.cursor()
    cur.execute("SELECT id, user_id, user_name, chat_id, chat_type, quiz_id, run_at FROM scheduled_quizzes")
    rows = cur.fetchall()
    conn.close()
    now = datetime.utcnow()
    for sid, user_id, user_name, chat_id, chat_type, quiz_id, run_at in rows:
        try:
            delay = (datetime.fromisoformat(run_at) - now).total_seconds()
        except ValueError:
            delay = 0
        _arm_schedule_job(application, sid, chat_id, chat_type, quiz_id, user_id, user_name or "Scheduler", delay)
    if rows:
        logger.info(f"Re-armed {len(rows)} pending scheduled quiz(zes) after startup.")
...
async def myschedules_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    cur = conn.cursor()
    cur.execute(
        "SELECT id, chat_id, run_at, quiz_title FROM scheduled_quizzes WHERE user_id = ? ORDER BY run_at",
        (user_id,),
    )
    rows = cur.fetchall()
    conn.close()
    ...
    if not rows:
        await update.effective_message.reply_text(
            "You have no pending scheduled quizzes.\nUse /schedule to create one."
        )
        return

    now = datetime.utcnow()
    lines = [f"📅 **{stylize('Your Scheduled Quizzes')}**\n_{FONT_STYLE}_\n"]
    keyboard = []
    for sid, chat_id, run_at, quiz_title in rows:
        try:
            mins_left = max(0, int((datetime.fromisoformat(run_at) - now).total_seconds() // 60))
            when = f"in ~{mins_left} min"
        except ValueError:
            when = "time unknown"
        lines.append(f"• #{sid} — {quiz_title or 'Quiz'} — {when} (chat `{chat_id}`)")
        keyboard.append([InlineKeyboardButton(f"❌ Cancel #{sid}", callback_data=f"schedcancel_{sid}")])
...
async def schedule_cancel_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    conn = db_connect()
    cur = conn.cursor()
    cur.execute("SELECT user_id FROM scheduled_quizzes WHERE id = ?", (schedule_id,))
    row = cur.fetchone()
    if not row or row[0] != user_id:
        conn.close()
        await query.edit_message_text("❌ Not found — it may have already run or been cancelled.")
        return
    cur.execute("DELETE FROM scheduled_quizzes WHERE id = ?", (schedule_id,))
    conn.commit()
    conn.close()
    ...
    await query.edit_message_text(f"🗑 Cancelled scheduled quiz #{schedule_id}.")
...
async def _post_init(application):
    """Runs once, right after the bot connects, before it starts polling for updates —
    the right place for 'catch up on what happened while we were down' startup work."""
    try:
        rearm_pending_schedules(application)
    except Exception as e:
        logger.error(f"rearm_pending_schedules failed: {e}", exc_info=True)
    try:
        await recover_interrupted_live_quizzes(application.bot)
    except Exception as e:
        logger.error(f"recover_interrupted_live_quizzes failed: {e}", exc_info=True)
...
def main():
    ...
    app.add_handler(CallbackQueryHandler(schedule_time_button, pattern="^schedtime_\\d+$"))
    app.add_handler(CallbackQueryHandler(schedule_quiz_button, pattern="^schedquiz_\\d+_\\d+$"))

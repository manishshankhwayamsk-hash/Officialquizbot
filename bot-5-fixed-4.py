...
def _build_system_prompt(count: int, difficulty: str, language: str) -> str:
    if language == "Both":
        language_block = (
            "LANGUAGE — read carefully: this is a BILINGUAL quiz. For EVERY item, write "
            "the question, all 4 options, AND the explanation in BOTH Hindi and English "
            "together — Hindi line first, then the English line right after it (e.g. "
            "question: \"<Hindi question>\\n<English question>\"), same pattern for every "
            "option and for the explanation. Never give only one language for any part. "
            "Proper nouns (names, places) may stay as commonly written in both lines."
        )
    else:
        language_block = (
...
async def ask_language(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    keyboard = InlineKeyboardMarkup([
        ...
        [InlineKeyboardButton("🌐 BOTH (Hindi + English)", callback_data=_tag_cb("lang_Both", owner_id))],
    ])

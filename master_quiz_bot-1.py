...
def answer_doubt_sync(user_id: int, question: str, doubt_context: str = "") -> str:
    ...
    for cand in chain:
        try:
            if cand["provider"] == "anthropic":
                return _ask_via_anthropic(cand["api_key"], cand["model"], question, doubt_context)
            elif cand["provider"] == "gemini":
                return _ask_via_gemini(cand["api_key"], cand["model"], question, doubt_context)
            else:
                return _ask_via_openai_compatible(cand["api_key"], cand["api_base"], cand["model"], question, doubt_context)
        except ProviderError as e:

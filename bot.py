...
async def send_typed(bot, chat_id, text: str, chunk_words: int = 3, delay: float = 0.35, **kwargs):
    words = text.split(" ")
...
async def send_typed_poll_intro(bot, chat_id, question_preview: str, hold: float = 0.3):
    try:
        placeholder = await send_typed(bot, chat_id, f"❓ {question_preview}"[:300])

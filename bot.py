...
OWNER_PROVIDER_KEYS = {
    ...
    "github": {"api_key": os.getenv("GITHUB_API_KEY", "")},
    "openrouter_text": {"api_key": os.getenv("OPENROUTER_API_KEY", "")},
    "openrouter_vision": {"api_key": os.getenv("OPENROUTER_API_KEY", "")},
...
OWNER_FALLBACK_ORDER = [
    ...
    "github", "openrouter_text", "openrouter_vision", "mistral", "huggingface", "openai",
]
...
def build_provider_chain(user_id: int) -> list:
    ...
    key the bot owner has configured via env vars (OWNER_PROVIDER_KEYS), tried
    in OWNER_FALLBACK_ORDER, with the owner's ANTHROPIC_API_KEY (if set) as the
    guaranteed last resort — so generation keeps working ("meri wali to run
    kregi") no matter how many keys a user has added, or whether they've added
    any at all, and no matter which of the owner's own keys happen to be
    ...
    for provider in OWNER_FALLBACK_ORDER:
        ...
        if preset.get("needs_base"):
            ...
            # from a second env var rather than a fixed preset base.
            account_id = owner_cfg.get("account_id", "")
            if not account_id:
                continue  # key without account id is unusable — skip silently
